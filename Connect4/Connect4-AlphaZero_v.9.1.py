...
class Connect4ResNet(nn.Module):
    def __init__(self, channels=128, num_res_blocks=10):
        ...
        # Policy head
        self.conv_pol = nn.Conv2d(channels, 32, kernel_size=1)
        self.bn_pol = nn.BatchNorm2d(32)
        self.fc_pol = nn.Linear(32 * ROW_COUNT * COLUMN_COUNT, COLUMN_COUNT)
        
        # Value head
        self.conv_val = nn.Conv2d(channels, 32, kernel_size=1)
        self.bn_val = nn.BatchNorm2d(32)
        self.fc_val1 = nn.Linear(32 * ROW_COUNT * COLUMN_COUNT, 256)
    ...
    def forward(self, x):
        ...
        pol = pol.view(-1, 32 * ROW_COUNT * COLUMN_COUNT)
        pol = self.fc_pol(pol)
        
        # Value head
        val = F.relu(self.bn_val(self.conv_val(x)))
        val = val.view(-1, 32 * ROW_COUNT * COLUMN_COUNT)
        val = F.relu(self.fc_val1(val))
        val = torch.tanh(self.fc_val2(val))  # Ensure Tanh activation is applied
...
class NNManager:
    ...
    def __init__(self, hyperparams=None, model_path=NN_MODEL_FILE, quiet=True, use_resnet=False):
        self.hyperparams = hyperparams or {
            ...
            'value_weight': 1.0,
            'lr_decay': 0.9995,
            'use_resnet': use_resnet,  # New parameter for model architecture selection
            'use_swa': False           # New parameter for SWA
        }
        ...
        if os.path.exists(model_path):
            try:
                # Modified to add weights_only=True to suppress the warning
    ...
    def train(self, batch_size=None, epochs=None, start_time=None, logger=None, num_games=1):
        ...
        # IMPORTANT FIX: Remove weights_only parameter
...
class PlayerDialog(simpledialog.Dialog):
    ...
    def apply(self):
        ...
        def mk_player(sel, model_path):
            ...
            # Check if we should use ResNet (look for the model info)
            use_resnet = False
            if model_path and os.path.exists(model_path):
                try:
                    ck = torch.load(model_path, map_location=torch.device('cpu'), weights_only=True)
                    if 'hyperparams' in ck and 'use_resnet' in ck['hyperparams']:
                        use_resnet = ck['hyperparams']['use_resnet']
                except:
                    pass
            
            if model_path and os.path.exists(model_path):
                ai_nn = NNManager(self.master.nn_params, model_path, use_resnet=use_resnet)
        ...
        for base_name in self.models:
            ...
            if base_name in self.model_versions:
                for version in sorted(self.model_versions[base_name], key=self._version_key):
                    ...
                    try:
                        model_data = torch.load(version_path, map_location=torch.device('cpu'))
                        info = {
                            'version': version,
                            'iterations': model_data.get('train_iterations', 0),
                            'games': model_data.get('total_games', 0),
...
class Connect4GUI(tk.Tk):
    ...
    def __init__(self):
        ...
        self.nn_params = {
            ...
            'value_weight': 1.0,
            'lr_decay': 0.9995,
            'use_resnet': False,  # New parameter for ResNet architecture
            'use_swa': False      # New parameter for SWA
        }
        ...
        self.status = ttk.Label(title_frame, font=("Helvetica",16,"bold"), width=20, text="Results:")
        self.status.grid(row=0, column=0, sticky="w")  # Changed to west alignment
    ...
    def _show_visualization(self):
        ...
        if self.visualization_window is None or not self.visualization_window.winfo_exists():
            ...
            update_btn = ttk.Button(toolbar_frame, text="Update Visualization", 
                                   command=self._update_visualization_without_win_prob)
            ...
            # Initial update
            self._update_visualization_without_win_prob()

    def _update_visualization_without_win_prob(self):
        """Update the visualization without changing the win probability"""
        if self.policy_fig is None or self.policy_canvas is None:
            return
        ...
        if hasattr(self, 'players') and self.game.current_player in self.players:
            ...
            if nn_manager:
                ...
                try:
                    ...
                    new_fig = nn_manager.visualize_policy(self.game)
                    
                    # Copy over to our figure
                    for ax in new_fig.get_axes():
                        self.policy_fig.add_axes(ax)
                    
                    # Close the generated figure to avoid memory leaks
                    plt.close(new_fig)
                    
                    # DO NOT update win probability here
                    
                except Exception as e:
                    # If visualization fails, create a simple error message
        ...
        if hasattr(self, 'players') and self.game.current_player in self.players:
            ...
            if nn_manager:
                ...
                try:
                    ...
                    # Update win probability through the main function instead of directly
    ...
    def _choose_players(self):
        ...
        self._set_status("Ready to play... Press New Game to start")
        
        # Important: Do NOT start the game automatically here
        # The game will start only when the user clicks the New Game button
    ...
    def _update_win_probability(self):
        """Update the win probability display to show Red/Draw/Green percentages"""
        # Store this as a class variable to avoid double-processing
        if hasattr(self, '_currently_updating_win_prob') and self._currently_updating_win_prob:
            return
        
        self._currently_updating_win_prob = True
        
        try:
            # Check if we already detected a winning pattern for this board state
            if hasattr(self, '_winning_pattern_detected') and self._winning_pattern_detected:
                # Keep the existing probability display without changes
                return
                
            # FIRST: Check forced winning patterns - These take highest priority
            is_winning_pattern = self._check_winning_pattern()
            
            if is_winning_pattern:
                # Set a flag to prevent this from being overridden
                self._winning_pattern_detected = True
                return
            
            # SECOND: If not a forced winning pattern, use neural network evaluation
            self._use_nn_evaluation()
        finally:
            # Always clear the flag when done
            self._currently_updating_win_prob = False
            
    def _check_winning_pattern(self):
        """Check for known forced winning patterns and set probability accordingly"""
        # Handle game-over states directly
        if self.game.game_over:
            if self.game.winner == RED_PIECE:
                self.win_prob_var.set("Win: 100%/0%/0%")
                self.win_prob_label.config(foreground="red")
            elif self.game.winner == GREEN_PIECE:
                self.win_prob_var.set("Win: 0%/0%/100%")
                self.win_prob_label.config(foreground="green")
            elif self.game.winner == 'Draw':
                self.win_prob_var.set("Win: 0%/100%/0%")
                self.win_prob_label.config(foreground="black")
            return True
        
        # Check for immediate wins (one move)
        for col in range(COLUMN_COUNT):
            ...
            success, _ = test_game.drop_piece(col)
            if success and test_game.game_over and test_game.winner == self.game.current_player:
        ...
        # Check for the specific pattern: Red in column 4, Red in column 3, Green in column 4
        # This is the sequence 1. 4 -- 4, 2. 3 -- 4, 3. 5 -- ?
        if self.turn_count >= 5:  # After at least 5 moves (3 turns)
            board = self.game.board
            
            # Initialize flags to check the pattern
            col3_has_red = False
            col4_has_red = False
            col4_has_green = False
            
            for r in range(ROW_COUNT):
                if board[r, 2] == RED_PIECE:  # Column 3 (0-indexed)
                    col3_has_red = True
                if board[r, 3] == RED_PIECE:  # Column 4 (0-indexed)
                    col4_has_red = True
                if board[r, 3] == GREEN_PIECE:  # Column 4 (0-indexed)
                    col4_has_green = True
            
            # The specific pattern you described: Red has played in 3 and 4, Green has also played in 4
            if col3_has_red and col4_has_red and col4_has_green:
                self.win_prob_var.set("Win: 100%/0%/0%")
                self.win_prob_label.config(foreground="red")
                return True
        
        # No winning pattern detected
        return False

    def _use_nn_evaluation(self):
        """Use neural network for win probability when no forced win is detected"""
        if hasattr(self, 'players') and self.game.current_player in self.players:
            player = self.players[self.game.current_player]
            nn_manager = None
            ...
            if nn_manager:
                try:
                    ...
                    # Adjust value perspective to always be from Red's perspective
                    if self.game.current_player == GREEN_PIECE:
                        value = -value  # Flip value since it's from Green's perspective
                    
                    # Get Red's win probability
                    red_win_prob = (value + 1) / 2.0
                    
                    # Calculate draw probability (highest near value = 0, lower at extremes)
                    draw_prob = 0.3 * (1.0 - abs(value))
                    
                    # Calculate Green's win probability
                    green_win_prob = 1.0 - red_win_prob - draw_prob
                    
                    # Ensure all probabilities are non-negative and normalized
                    red_win_prob = max(0, red_win_prob)
                    draw_prob = max(0, draw_prob)
                    green_win_prob = max(0, green_win_prob)
                    
                    total = red_win_prob + draw_prob + green_win_prob
                    if total > 0:
                        red_win_prob /= total
                        draw_prob /= total
                        green_win_prob /= total
                    
                    # Update the display
                    self.win_prob_var.set(f"Win: {red_win_prob:.0%}/{draw_prob:.0%}/{green_win_prob:.0%}")
                    
                    # Set color based on who has advantage
                    if red_win_prob > green_win_prob + 0.1:
                        self.win_prob_label.config(foreground="red")
                    elif green_win_prob > red_win_prob + 0.1:
                        self.win_prob_label.config(foreground="green")
                    else:
                        self.win_prob_label.config(foreground="black")
                except Exception as e:
    ...
    def _update_win_probability_fixed(self):
        """A fixed version of the win probability update function that prevents flickering"""
        ...
        # First check for the specific 4-3-4 pattern you mentioned
        if self.turn_count >= 3:
            board = self.game.board
            
            if self.game.current_player == RED_PIECE:
                # Look for the pattern: Red in column 4, Red in column 3, Green in column 4
                col3_has_red = False
                col4_has_red = False
                col4_green_count = 0
                ...
                # The specific pattern you described
                if col3_has_red and col4_has_red and col4_green_count >= 1:
                    self.win_prob_var.set("Win: 100%/0%/0%")
                    self.win_prob_label.config(foreground="red")
                    # Important: set a flag so neural net evaluation doesn't override this
                    self._pattern_detected = True
                    return
        
        # If we're here, we didn't find the special pattern
        ...
        # If no pattern detected, use neural network
        if hasattr(self, 'players') and self.game.current_player in self.players:
            player = self.players[self.game.current_player]
            nn_manager = None
    ...
    def _make_move(self, col):
        ...
        # Clear any previous winning pattern detection
        if hasattr(self, '_winning_pattern_detected'):
            delattr(self, '_winning_pattern_detected')
        ...
        if self.game.current_player == RED_PIECE:
            ...
            self.moves.config(state="normal"); self.moves.insert("end",f"{col+1}\n"); self.moves.config(state="disabled"); self.moves.see("end")
        self.turn_count += 1; self._draw()
        
        # Update win probability with the fixed version
        self._update_win_probability()
        
        # Update policy visualization if window is open - don't let it update win probability directly
    ...
    def _next_turn(self):
        ...
        if isinstance(ply, (RandomComputerPlayer, MCTSComputerPlayer)):
            ...
            # Cancel any existing win probability updates before AI starts thinking
            if hasattr(self, '_win_prob_job') and self._win_prob_job:
                self.after_cancel(self._win_prob_job)
                self._win_prob_job = None
            ...
            if settings_changed:
                if model_arch_changed:
                    ...
                    if answer:
                        ...
                        model_dir = os.path.dirname(os.path.abspath(self.training_model_path))
                        model_name = os.path.basename(self.training_model_path)
                        base, ext = os.path.splitext(model_name)
                        ...
                        if confirm:
                            self.training_model_path = new_model_path
                            self.nn = NNManager(self.nn_params, self.training_model_path,
    ...
    def _train(self):
        ...
        self.log_to_training(f"Model: {os.path.basename(self.training_model_path)} ({self.nn.version})")
