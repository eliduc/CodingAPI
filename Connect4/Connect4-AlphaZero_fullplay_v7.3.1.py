...
Version 8.2 - UI improvements and additional training metrics
...
class PlayerDialog(simpledialog.Dialog):
    def __init__(self, master, mcts_params, nn):
        ...
        self.fullplay_var = tk.BooleanVar(master, False)        # Moved from main window
...
class Connect4GUI(tk.Tk):
    ...
    def _choose_players(self):
        # If there's already a full strength variable in the main GUI, we need to pass its value to the dialog
        # We don't have it anymore since we removed it from the main window
        fullplay_var_value = False  # Default value
    ...
    def _training_worker_thread(self, n):
        ...
        try:
            ...
            # Important: use fullplay_var value to control exploration during training
            # Since fullplay_var is now in the PlayerDialog, we don't have direct access to it here
            # We'll use the default exploration mode (True) for training
            game_func = partial(
                _play_single_training_game, 
                self.mcts_params['iterations'], 
                self.mcts_params['C_param'], 
                self.nn,
                True  # exploration enabled for training
            )
