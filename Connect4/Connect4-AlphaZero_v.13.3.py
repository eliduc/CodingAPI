...
class Connect4CNN(nn.Module):
    def __init__(self, channels=96, dropout_rate=0.2, fc_dropout_rate=0.4):
        super().__init__()
        # Fix padding from 2 to 1 to maintain spatial dimensions
        self.conv1 = nn.Conv2d(3, channels, kernel_size=4, padding=1)
        self.conv2 = nn.Conv2d(channels, channels, kernel_size=4, padding=1)
        self.conv3 = nn.Conv2d(channels, channels, kernel_size=4, padding=1)
        self.conv4 = nn.Conv2d(channels, channels, kernel_size=4, padding=1)
        self.bn1 = nn.BatchNorm2d(channels)
        self.bn2 = nn.BatchNorm2d(channels)
...
class TreeNode:
    ...
    def u(self, c_puct, total_visits):
        ...
        epsilon = 1e-8
        return c_puct * self.prior * math.sqrt(total_visits) / (1 + self.visits + epsilon)

    def best_child(self, c_puct):
        # Add small epsilon to prevent division by zero on first simulation
        epsilon = 1e-8
        total = max(self.visits, epsilon)
...
class MCTS:
    ...
    def search(self, root_state: 'Connect4Game', previous_root=None, move_number=0):
        ...
        # Temperature decay after move 10 - allow more deterministic late-game play
        # with a lower floor (0.05 instead of 0.1)
        if move_number > 10:
        ...
        # Calculate adaptive iterations based on win rate with a cap
        ...
        if hasattr(self.nn, 'win_rate'):
            ...
            # Cap the win rate factor to prevent runaway calculations
            win_rate = min(0.95, max(0.0, self.nn.win_rate))  # Cap at 95%
            win_rate_factor = 1.0 + max(0, (win_rate - 0.5) * 2.0)
            # Cap the factor to prevent excess computation
            win_rate_factor = min(2.0, win_rate_factor)
    ...
    def _normalize_reused_root(self):
        """
        Improved adaptive tree-reuse normalization to prevent over-dampening the root.
        Uses a more gradual approach based on the ratio of child visits to root visits.
        """
        if not hasattr(self, 'root') or self.root is None:
            return
        
        # Calculate child visit sum
        child_visit_sum = sum(child.visits for child in self.root.children.values())
        
        # Only normalize if there's a significant imbalance
        if self.root.visits > 0 and child_visit_sum > 0:
            # Calculate the ratio of child visits to root visits
            visit_ratio = child_visit_sum / self.root.visits
            
            # Use an adaptive scaling approach:
            # - If ratio is very low (<0.2), more aggressive scaling
            # - If ratio is moderate (0.2-0.6), gentle scaling
            # - If ratio is good (>0.6), minimal adjustment
            if visit_ratio < 0.2:
                # More aggressive scaling for very imbalanced trees
                target_visits = max(50, child_visit_sum / 0.4)
                self.root.visits = min(self.root.visits, target_visits)
            elif visit_ratio < 0.6:
                # Gentle scaling for moderately imbalanced trees
                target_visits = max(30, child_visit_sum / 0.6)
                self.root.visits = min(self.root.visits, target_visits)
            # Else leave the visits as they are - the tree is balanced enough
