...
class Connect4Dataset(Dataset):
    ...
    def get_weights(self):
        """Return sample weights for weighted sampling"""
...
class NNManager:
    ...
    def finish_game(self, winner):
        ...
        # Batch evaluation of all pending positions
        batch_states = []
        batch_indices = []
        ...
        # Batch predict values for all states at once
        ...
        for i, ex in enumerate(self.pending):
            if winner == 'Draw':
                ...
                # Limit value to 1.0 (max tanh output) instead of 1.2
                value = 1.0  # Fixed: Capped at 1.0 to match tanh range
                win_count += 1
            else:
                # Limit value to -1.0 (min tanh output) instead of -1.2
                value = -1.0  # Fixed: Capped at -1.0 to match tanh range
                loss_count += 1
                    
            # Use pre-computed value from batch prediction
            ...
            # Give additional priority to non-draw positions
            if winner != 'Draw':
                surprise *= 1.5  # Increase priority for decisive games
        ...
        if len(self.data['states']) > 1000:
            ...
            # IMPROVED: Less aggressive pruning, especially for center moves
            # Only prune if one column has >4x (instead of 3x) representation
            if np.max(move_counts) > 4 * np.min(move_counts) and len(unique_moves) == COLUMN_COUNT:
                avg_count = np.mean(move_counts)
                under_rep_cols = [col for i, col in enumerate(unique_moves) if move_counts[i] < avg_count * 0.6]
                over_rep_cols = [col for i, col in enumerate(unique_moves) if move_counts[i] > avg_count * 1.4]
                
                # Special protection for center column (typically column 3)
                center_col = COLUMN_COUNT // 2
                if center_col in over_rep_cols:
                    # Less aggressive pruning for center column (30% vs 50% for others)
                    center_idx = np.where(unique_moves == center_col)[0][0]
                    center_weight = move_counts[center_idx] / avg_count
                    center_drop_prob = min(0.3, (center_weight - 1.4) * 0.2)
                    
                    # Adjust pruning for other columns
                    other_drop_prob = 0.5
                    ...
                    for i, max_idx in enumerate(max_indices):
                        ...
                        elif max_idx == center_col:
                            if random.random() > center_drop_prob:
                                indices_to_keep.append(i)
                        elif max_idx in over_rep_cols:
                            if random.random() > other_drop_prob:
                                indices_to_keep.append(i)
                        else:
                            indices_to_keep.append(i)
        ...
        if len(self.data['states']) > MAX_TRAINING_EXAMPLES:
            ...
            if hasattr(self, 'data') and 'priorities' in self.data and self.data['priorities']:
                ...
                if 'values' in self.data:
                    ...
                    draw_indices = [i for i, v in enumerate(values) if abs(v) < 0.1]  # Find draws
                    non_draw_indices = [i for i, v in enumerate(values) if abs(v) >= 0.1]  # Find non-draws
                    
                    # If we have too many draws, replace some with non-draws
                    draw_count = sum(1 for i in keep_indices if i in draw_indices)
                    if draw_count > target_size * 0.3 and non_draw_indices:  # Cap draws at 30%
                        # Calculate how many draws to replace
                ...
                # FIXED: Prioritize newer data (20% recent, 60% middle, 20% oldest)
                keep_recent = int(target_size * 0.2)  # Keep same percentage of recent data
                keep_old = int(target_size * 0.2)    # Keep same percentage of old data
                keep_middle = target_size - keep_recent - keep_old
                
                # FIXED: Weight newer data higher than older data (reversed from original)
                ...
                if middle_size > keep_middle and keep_old > 0:
                    ...
                    if middle_size > keep_middle:
                        ...
                        middle_weights = np.linspace(1.0, 2.0, middle_size)  # Linearly increasing weights
    ...
    # Modified function to capture and display network training details
    def train(self, batch_size=None, epochs=None, start_time=None, logger=None, num_games=1):
        ...
        # Enhanced dynamic weighting of policy vs value losses
        policy_weight = self.hyperparams.get('policy_weight', 1.5)
        value_weight = self.hyperparams.get('value_weight', 1.0)
        
        # Apply auto-adjustment to weights based on training history
        if hasattr(self, 'training_losses') and 'policy' in self.training_losses and 'value' in self.training_losses:
            policy_losses = self.training_losses['policy']
            value_losses = self.training_losses['value']
            
            if len(policy_losses) >= 5 and len(value_losses) >= 5:
                # Check recent loss trends
                recent_p_losses = policy_losses[-5:]
                recent_v_losses = value_losses[-5:]
                
                p_trend = recent_p_losses[-1] - recent_p_losses[0]
                v_trend = recent_v_losses[-1] - recent_v_losses[0]
                
                # Dynamically adjust weights based on trends
                if p_trend > 0 and (v_trend <= 0 or abs(p_trend) > abs(v_trend) * 1.5):
                    # Policy loss increasing while value stable/decreasing
                    policy_weight = min(3.0, policy_weight * 1.2)
                    log(f"Increasing policy weight to {policy_weight:.2f} (policy loss trend: {p_trend:.6f})")
                elif v_trend > 0 and (p_trend <= 0 or abs(v_trend) > abs(p_trend) * 1.5):
                    # Value loss increasing while policy stable/decreasing
                    value_weight = min(3.0, value_weight * 1.2)
                    log(f"Increasing value weight to {value_weight:.2f} (value loss trend: {v_trend:.6f})")
        ...
        if use_data_augmentation:
            ...
            for i in range(original_data_size):
                ...
                # Add horizontally flipped sample (valid for Connect4)
                # Get state tensor and flip columns
                state = self.data['states'][i]
                flipped_state = torch.flip(state, [2])  # Flip along width dimension
                
                # Flip policy vector (column probabilities)
                policy = self.data['policies'][i]
                flipped_policy = torch.flip(policy, [0])  # Flip along column dimension
                
                # Value stays the same after horizontal flip
                value = self.data['values'][i]
                
                # Add the flipped sample
            ...
            log(f"Applied data augmentation with horizontal flipping: {original_data_size} → {len(aug_states)} examples")
        ...
        # Initialize cosine annealing LR scheduler if configured
        use_cosine_scheduler = self.hyperparams.get('use_cosine_scheduler', False)
        if use_cosine_scheduler:
            # Set up cosine annealing scheduler that cycles within the epoch
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                self.opt,
                T_max=epochs,
                eta_min=self.hyperparams.get('min_lr', 1e-6)
            )
            log(f"Using cosine annealing learning rate scheduler")
        ...
        for ep in range(epochs):
            ...
            for s, p_target, v_target in dl:
                ...
                if self.use_mixed_precision:
                    with torch.cuda.amp.autocast():
                        ...
                        # Mask invalid moves during training
                        # Create a batch of game states to find valid moves
                        batch_games = []
                        for i in range(s.size(0)):
                            game = Connect4Game()
                            s_np = s[i].cpu().numpy()
                            game.board = (s_np[0] > 0.5).astype(np.int8) * RED_PIECE + (s_np[1] > 0.5).astype(np.int8) * GREEN_PIECE
                            game.current_player = RED_PIECE if s_np[2][0, 0] > 0.5 else GREEN_PIECE
                            batch_games.append(game)
                        
                        # Create a mask for each example in the batch
                        masks = []
                        for game in batch_games:
                            valid_moves = game.valid_moves()
                            mask = torch.full((COLUMN_COUNT,), -1e9, device=self.device)
                        ...
                        # Add detailed debug output throughout training
                        if ep == 0 and batch_idx == 0:
                            ...
                            batch_example = p_target[0].detach().cpu().numpy()
                            log(f"Policy target example: {np.round(batch_example, 3)}")
                    ...
                    # Add detailed debug output throughout training
                    if ep == 0 and batch_idx % max(1, len(dl) // 2) == 0:
                        log(f"Batch {batch_idx} - Value pred: {v_pred.min().item():.3f} to {v_pred.max().item():.3f}")
                ...
                # Step cosine scheduler if enabled
                if use_cosine_scheduler:
                    scheduler.step()
            ...
            # Step the scheduler with total loss average
        ...
        # Store loss trends for dynamic weighting in future training
        ...
        # Keep only the most recent 50 epoch losses to avoid memory bloat
        max_history = 50
        ...
        save_dict = {
            ...
            'scheduler_state_dict': self.scheduler.state_dict(),  # Add scheduler state
            'train_iterations': self.train_iterations,
            'total_games': self.total_games,
            'version': self.version,
            'win_rate': self.win_rate,
            'hyperparams': self.hyperparams,
            'training_losses': self.training_losses  # Save loss history
        }
...
class TreeNode:
    ...
    def u(self, c_puct, total_visits):
        # Improved PUCT formula with better numerical stability
        # The epsilon ensures proper scaling even with very small values
        epsilon = 1e-6
        return c_puct * self.prior * math.sqrt(total_visits + epsilon) / (1 + self.visits)

    def best_child(self, c_puct):
        # Improved parent visit counting to ensure proper exploration
        # Using max(1, self.visits) ensures we never divide by zero
        total = max(1, self.visits)
        return max(self.children.values(), key=lambda n: n.q() + n.u(c_puct, total))

class MCTS:
    def __init__(self, iterations, c_puct, nn: NNManager, explore=True, dirichlet_noise=0.3, safe_bypass_prob=0.05):
        self.I = iterations
        self.c = c_puct
        self.nn = nn
        self.explore = explore
        self.dirichlet_noise = dirichlet_noise  # Store dirichlet_noise parameter
        self.safe_bypass_prob = safe_bypass_prob  # Configurable probability to bypass safe move logic
    ...
    def search(self, root_state: 'Connect4Game', previous_root=None, move_number=0):
        ...
        # Enhanced temperature decay - more gradual approach
        if move_number > 8:  # Start decay earlier at move 8 instead of 10
            # More sophisticated temperature decay with move number
            decay_factor = min(0.95, 0.98 - move_number * 0.005)  # Accelerating decay
        ...
        # FIX: Occasionally disable safe move logic during self-play for better exploration
        # Use configurable probability to bypass the safe move logic when in explore mode
        bypass_safe_logic = self.explore and random.random() < self.safe_bypass_prob
        
        # Safe move detection with deeper evaluation
        safe_moves = []
        if not bypass_safe_logic:
            for move in valid_moves:
                test_state = root_state.copy()
                test_state.drop_piece(move)
                test_state.switch()  # Switch to opponent's perspective
                
                # First level check - immediate win detection
                ...
                # FIX: Add a second level of depth - check if opponent can set up a winning move
                if not opponent_can_win and len(test_state.valid_moves()) > 0:
                    # Look ahead two plies for opponent to set up a guaranteed win
                    for opp_move in test_state.valid_moves():
                        test_state2 = test_state.copy()
                        test_state2.drop_piece(opp_move)
                        if not test_state2.game_over:
                            test_state2.switch()  # Switch to our player
                            
                            # Check if we have a forced move to prevent loss
                            forced_move = None
                            for our_move in test_state2.valid_moves():
                                test_state3 = test_state2.copy()
                                test_state3.drop_piece(our_move)
                                test_state3.switch()  # Switch to opponent
                            ...
                            if forced_move is None:
                                # No move prevents opponent from winning next turn
                                opponent_can_win = True
                                break
        ...
        if safe_moves and not bypass_safe_logic:
            # IMPROVED: Use neural network for safe moves but remove center bias influence
            if len(safe_moves) > 1:
                best_value = -float('inf')
                best_move = safe_moves[0]
                
                # Get base policy for all moves
                ...
                for move in safe_moves:
                    ...
                    # FIX: Switch player before evaluation to get correct perspective
                    test_state.switch()
                    
                    # Get value from neural network from proper perspective
                    _, value = self.nn.policy_value(test_state)
                    
                    # FIX: Negate value since we're now evaluating from opponent's perspective
                    value = -value
                    
                    # Consider both value and policy in selection without center bias
        ...
        # Enhanced full-strength and curriculum-based simulation count
        adaptive_iterations = self.I  # Start with default iteration count
        
        # Different handling for exploration vs full-strength modes
        if not self.explore:
            # In full-strength mode, use more simulations and lower exploration constant
            adaptive_iterations = int(self.I * 1.5)  # 50% more simulations in full-strength
            c_puct_value = 0.8  # Lower exploration parameter for full-strength
        else:
            # Curriculum learning: scale iterations based on win rate
            if hasattr(self.nn, 'win_rate'):
                win_rate = min(0.9, max(0.1, self.nn.win_rate))  # Constrain win rate for stability
                # Scale iterations - more compute as the model gets stronger
                # Map win_rate from [0.1, 0.9] to iteration multiplier [0.8, 2.0]
                iter_scale = 0.8 + (win_rate - 0.1) * 1.5
                adaptive_iterations = int(self.I * iter_scale)
                # Cap iterations to prevent excessive computation
                adaptive_iterations = min(adaptive_iterations, self.I * 2)
            c_puct_value = self.c  # Use standard exploration param for explore mode
        ...
        if (previous_root is not None and 
            ...
            if (opponent_last_move is not None and 
                ...
                if (self.root.state.board == root_state.board).all():
                    ...
                    # Scale down the visit count to avoid exploration bias
                    self._normalize_reused_root()
                    
                    # FIX: When reusing the root, apply fresh Dirichlet noise to child priors
                    if self.explore:
                        valid = root_state.valid_moves()
                        dirichlet = np.random.dirichlet([self.dirichlet_noise] * len(valid))
                        
                        # Apply noise to each child
                        for i, m in enumerate(valid):
                            if m in self.root.children:
                                # Apply 75-25 mix of original prior and new noise
        ...
        # Apply center bias ONCE to the prior and store as original_priors
        # Apply fading center bias that decreases as move number increases
        bias_strength = max(0.0, 1.0 - (move_number / 20.0))  # Fades to zero by move 20
        
        biased_priors = np.zeros(COLUMN_COUNT, dtype=np.float32)
        for m in valid:
            # Apply center bias with strength that fades over time
            if bias_strength > 0:
        ...
        # Store original priors before applying Dirichlet noise
        ...
        if self.explore:
            ...
            for i, m in enumerate(valid):
                # Apply noise to the already biased priors (not to raw priors)
                child_priors[m] = 0.75 * biased_priors[m] + 0.25 * dirichlet[i]
        else:
            # Use the biased priors directly without noise
        ...
        for m in valid:
            # Only create children that don't already exist
        ...
        # Run MCTS simulations with the adaptive iteration count
        for _ in range(adaptive_iterations):
            node = self.root
            # Select phase - navigate to leaf using best_child
            while node.children:
                node = node.best_child(c_puct_value)  # Use appropriate c_puct value
                
            # Expansion phase - only for non-terminal states
            if not node.state.game_over:
                probs, value = self.nn.policy_value(node.state)
                valid = node.state.valid_moves()
                
                # Apply center bias to the child nodes' priors only once here
                child_priors = np.zeros(COLUMN_COUNT, dtype=np.float32)
                for m in valid:
                ...
                # For terminal nodes, determine value from game result
        ...
        # For the first 8 moves, sample proportionally to visit counts to improve exploration
        if move_number < 8 and self.explore:
            # Sample move based on visit distribution to increase opening diversity
            valid_moves = [m for m in valid if visit_counts[m] > 0]
            if valid_moves:  # Safety check to ensure there are moves with non-zero visits
                move_probs = visit_counts[valid_moves] / visit_counts[valid_moves].sum()
                best_move = int(np.random.choice(valid_moves, p=move_probs))
            else:
                # Fallback to argmax if all visits are zero (shouldn't happen, but for safety)
                best_move = int(visit_counts.argmax())
        else:
            # For later moves, use argmax as before
            best_move = int(visit_counts.argmax())
    ...
    def _normalize_reused_root(self):
        """
        Enhanced adaptive tree reuse normalization that dynamically scales
        the root visit count based on the relationship between parent and
        child visit totals. This prevents exploitation bias in reused subtrees.
        """
        if not hasattr(self, 'root') or self.root is None:
            return
        
        # Calculate total visits in child nodes
        child_visit_sum = sum(child.visits for child in self.root.children.values())
        
        # Skip normalization if we don't have meaningful visit data
        if self.root.visits <= 0 or child_visit_sum <= 0:
            return
            
        # Calculate the ratio of child visits to root visits
        visit_ratio = child_visit_sum / self.root.visits
        
        # Apply multi-tier adaptive scaling based on visit ratio
        if visit_ratio < 0.15:
            # Extreme imbalance - aggressive normalization
            scale_factor = 0.2
            target_visits = max(80, child_visit_sum / scale_factor)
            self.root.visits = min(self.root.visits, int(target_visits))
        elif visit_ratio < 0.3:
            # Significant imbalance - moderate normalization
            scale_factor = 0.4
            target_visits = max(50, child_visit_sum / scale_factor)
            self.root.visits = min(self.root.visits, int(target_visits))
        elif visit_ratio < 0.6:
            # Moderate imbalance - light normalization
            scale_factor = 0.7
            target_visits = max(30, child_visit_sum / scale_factor)
            self.root.visits = min(self.root.visits, int(target_visits))
        
        # Ensure value sum is consistent with the new visit count
        # This maintains the same Q value after normalization
        q_value = self.root.value_sum / max(1, self.root.visits)
        self.root.value_sum = q_value * self.root.visits
        
        # Renormalize child prior probabilities to ensure they sum to 1
        valid_moves = self.root.state.valid_moves()
        prior_sum = sum(self.root.children[m].prior for m in valid_moves if m in self.root.children)
        
        if prior_sum > 0 and abs(prior_sum - 1.0) > 1e-3:
...
class MCTSComputerPlayer(Player):
    def __init__(self, iters, c, nn: NNManager, explore=True, model_path=None, dirichlet_noise=0.3, safe_bypass_prob=0.05):
        self.nn = nn
        self.model_path = model_path
        # Consistent higher c_puct for full-strength mode (increased from 0.6)
        self.mcts = MCTS(iters, c if explore else 0.8, nn, explore=explore, dirichlet_noise=dirichlet_noise, safe_bypass_prob=safe_bypass_prob)
    ...
    def reset(self):
        ...
        if hasattr(self, 'mcts') and self.mcts is not None:
            ...
            # Reset temperature to ensure fresh games start with proper exploration
...
class Connect4GUI(tk.Tk):
    ...
    def _toggle_visualization(self):
        """Toggle visualization window on/off"""
        try:
    ...
    def _switch_visualization_player(self):
        """Switch between Red and Green player visualization"""
    ...
    def _update_switch_button_text(self):
        """Update the switch button text based on current visualization player"""
        if self.current_vis_player == RED_PIECE:
            self.switch_player_btn.config(text="Switch to Green")
        else:
            self.switch_player_btn.config(text="Switch to Red")
    ...
    def _show_visualization(self):
        """Create and show the policy visualization window"""
        try:
    ...
    def _update_visualization(self, force_game_over=False):
        ...
        try:
            ...
            if not hasattr(self, 'visualization_frame') or not self.visualization_frame.winfo_ismapped():
                return
            ...
            # KEY FIX: Handle game over state - don't query neural network on terminal positions
            if self.game.game_over or force_game_over:
                # Draw final game state visualization without querying the neural network
                ax = self.policy_fig.add_subplot(111)
                ...
                if self.game.winner == 'Draw':
                    ...
                    value_text = "Value: 0.000"
                    red_win_prob = 0.0
                    green_win_prob = 0.0
                elif self.game.winner == RED_PIECE:
                    # Red wins
                    title = "Red Wins"
                    ax.set_title(title, fontsize=14, color='red', pad=10)
                    value_text = "Value: 1.000"
                    red_win_prob = 1.0
                    green_win_prob = 0.0
                else:
                    # Green wins
                    title = "Green Wins" 
                    ax.set_title(title, fontsize=14, color='green', pad=10)
                    value_text = "Value: -1.000"
                    red_win_prob = 0.0
                    green_win_prob = 1.0
                
                # Show empty policy (no moves available)
                bar_positions = np.arange(1, COLUMN_COUNT + 1)
                probs = np.zeros(COLUMN_COUNT)
                bars = ax.bar(bar_positions, probs, color='gray', width=0.6)
                
                ax.set_ylim(0.0, 0.2)  # Fixed scale for empty policy
                ...
                # Draw text indicating game over
                ax.text(
                    COLUMN_COUNT/2, 0.1,
                ...
                ax.text(
                    4.0, -0.045,  # Center position, same level as Red/Green boxes
                    value_text,
                    ha='center', va='center',  # Changed alignment to center
                    bbox=dict(facecolor='white', alpha=0.9, boxstyle='round', pad=0.3),
                    fontsize=10,  # Matched fontsize with other boxes
                    transform=ax.transData
                )
                
                # Show win probabilities at bottom
                ax.text(
                    1.5, -0.1,
                    f"Red: {red_win_prob:.1%}",
                    ha='center', va='center',
                ...
                ax.text(
                    6.5, -0.1,
                    f"Green: {green_win_prob:.1%}",
                    ha='center', va='center',
                    bbox=dict(facecolor='green', alpha=0.3, boxstyle='round', pad=0.2),
                    fontsize=10,
                    transform=ax.transData
                )
                ...
                self.policy_fig.subplots_adjust(bottom=0.2)
                self.policy_canvas.draw()
                return
                    
            # Get policy and value from neural network - for non-terminal states
            if hasattr(self, 'nn'):
                current_state = self.game
                
                # Use the current player's neural network if it's an AI player
                nn_manager = self.nn  # Default
                if self.game.current_player in self.players:
                    player = self.players[self.game.current_player]
                    if isinstance(player, MCTSComputerPlayer):
                        nn_manager = player.nn
                
                probs, value = nn_manager.policy_value(current_state)
                
                # Always display the value from current player's perspective
                if current_state.current_player == RED_PIECE:
                    red_win_prob = (value + 1) / 2.0
                else:
                    red_win_prob = (1 - value) / 2.0
                    
                green_win_prob = 1.0 - red_win_prob
                
                # Store probabilities for consistent UI
                self.current_red_win_prob = red_win_prob
                self.current_green_win_prob = green_win_prob
                self.current_draw_prob = 0.0
                        
                # Create a new axis for the policy distribution
                ax = self.policy_fig.add_subplot(111)
                        
                # Create the bars
                bar_positions = np.arange(1, COLUMN_COUNT + 1)
                bars = ax.bar(bar_positions, probs, color='red' if current_state.current_player == RED_PIECE else 'green', width=0.6)
                        
                # Add labels for move probabilities on top of each bar
                ...
                # Set axes properties
                max_height = max(0.2, max(probs) * 1.2)
                ax.set_ylim(0.0, max_height)
                ax.set_xlim(0.25, COLUMN_COUNT + 0.75)  # Ensure all bars are visible
                ax.set_xticks(bar_positions)
                ax.set_xticklabels([str(i) for i in range(1, COLUMN_COUNT + 1)])
                
                # Keep Y-axis label but make it compact
                ax.set_ylabel('Prob', fontsize=10, labelpad=0)
                
                # Reduce title size and add more space at the top
                ax.set_title(f"{'Red' if current_state.current_player == RED_PIECE else 'Green'}'s Move Distribution", 
                          fontsize=14, color='red' if current_state.current_player == RED_PIECE else 'green',
                          pad=10)
                
                # Add grid lines but make them less prominent
                ax.grid(True, linestyle='--', alpha=0.5, axis='y')
                
                # Use simplified Value display
                value_text = f"Value: {value:.3f}"
                    
                ax.text(
                    4.0, -0.22 * max_height,  # Center position, same level as Red/Green boxes
                    value_text,
                    ha='center', va='center',  # Changed alignment to center
                    bbox=dict(facecolor='white', alpha=0.9, boxstyle='round', pad=0.3),
                    fontsize=10,  # Matched fontsize with other boxes
                ...
                # Add probabilities in a row at the bottom - without draw probability
                # Red win probability (left)
                ax.text(
                    1.5, -0.22 * max_height,
                    f"Red: {red_win_prob:.1%}",
                    ha='center', va='center',
                ...
                # Maximize the plot area within the figure
                self.policy_fig.tight_layout()
                
                # Add extra space at the bottom to fit our text elements
                self.policy_fig.subplots_adjust(bottom=0.2)
            ...
            # Display error message in visualization and print traceback
            import traceback
            traceback.print_exc()
    ...
    def destroy(self):
        """Override Tk.destroy so the training log is flushed before exit."""
        # If we have not yet saved the log in this session, do it now.
    ...
    def _save_log_on_exit(self):
        """Save the training log to disk and close the application cleanly."""
        try:
    ...
    def _new_game(self):
        ...
        # Initialize win probabilities - near 50/50 at start
        self.current_red_win_prob = 0.5
        self.current_green_win_prob = 0.5
        self.current_draw_prob = 0.0
        ...
        # Update win probability based on neural network
    ...
    def _choose_players(self):
        # Store current window and widget dimensions
        current_width = self.winfo_width()
        current_height = self.winfo_height()
        
        # Store history frame width if it exists
        hist_width = 0
        if hasattr(self, 'moves') and self.moves.winfo_exists():
            hist_width = self.moves.winfo_width()
        ...
        # Ensure window layout is preserved - force the window geometry
        self.update_idletasks()
        
        # Reset original dimensions if they were valid
        if current_width > 100 and current_height > 100:
            self.geometry(f"{current_width}x{current_height}")
        
        # Force game history width if it was previously defined
        if hist_width > 0 and hasattr(self, 'moves') and self.moves.winfo_exists():
            self.moves.config(width=max(20, hist_width))
    ...
    def log_to_training(self, *args, **kwargs):
        """Add message to training log, both terminal and GUI"""
    ...
    def _update_log_from_buffer(self):
        """Update training log GUI from buffer at regular intervals"""
        try:
    ...
    def _update_win_probability(self):
        """Update win probability display, ensuring consistency with visualization"""
            ...
            if nn_manager:
                try:
                    ...
                    if self.game.game_over:
                        ...
                        # Store probabilities for consistency
                        self.current_red_win_prob = red_win_prob
                        self.current_green_win_prob = green_win_prob
                        self.current_draw_prob = 0.0
                        
                        # Update Win Probs display with game result
                        self.win_prob_var.set(f"Win Probs: Red: {red_win_prob:.0%}/Green: {green_win_prob:.0%}")
                        
                        # Set label color based on winner
                        if red_win_prob > 0.5:
                            self.win_prob_label.config(foreground="red")
                        elif green_win_prob > 0.5:
                            self.win_prob_label.config(foreground="green")
                        else:
                            self.win_prob_label.config(foreground="black")
                        
                        # KEY FIX: Don't update visualization on game over - would cause incorrect values
                        if self.visualization_visible and self.auto_update_var.get():
                            self._update_visualization(force_game_over=True)  # Pass flag to handle game over specially
                        
                        return  # Exit early after handling game over state
                    else:
                        # Check for immediate win on next move
                        current_player = self.game.current_player
                        opponent = GREEN_PIECE if current_player == RED_PIECE else RED_PIECE
                        
                        # Look for winning move for current player
                        can_win = False
                        ...
                        if can_win:
                            ...
                            # Calculate from neural network
                            _, value = nn_manager.policy_value(self.game)
                            
                            # Convert value to win probability from perspective of Red player
                            if self.game.current_player == RED_PIECE:
                                red_win_prob = (value + 1) / 2.0
                            else:
                                red_win_prob = (1 - value) / 2.0
                            
                            # Set Green's win probability as complement (no draw)
                            green_win_prob = 1.0 - red_win_prob
                    
                    # Update Win Probs display
                    self.win_prob_var.set(f"Win Probs: Red: {red_win_prob:.0%}/Green: {green_win_prob:.0%}")
                    
                    # Store probabilities for consistent visualization
                    self.current_red_win_prob = red_win_prob
                    self.current_green_win_prob = green_win_prob
                    self.current_draw_prob = 0.0
                    
                    # Set label color based on advantage
                    if red_win_prob > 0.6:
                        self.win_prob_label.config(foreground="red")
                    elif green_win_prob > 0.6:
                        self.win_prob_label.config(foreground="green")
                    else:
                        self.win_prob_label.config(foreground="black")
                        
                    # Update visualization if it's visible and auto-update is enabled
                    if self.visualization_visible and self.auto_update_var.get():
                        self._update_visualization()
                        
                except Exception as e:
                    print(f"Error updating win probability: {e}")
    ...
    def _process_collected_games(self, all_examples, all_game_results, start_game, end_game, elapsed_str):
        try:
            ...
            if not hasattr(self, 'force_stop_training') or not self.force_stop_training:
                ...
                # Add a header but don't clear existing content
    ...
    def _perform_end_game_training(self):
        ...
        # FIXED: Check if Learn@Play is enabled and process if human was playing
        if (not has_random_player) and (self.learn_var.get() or self.is_comp):
            ...
            # FIXED: Make sure AI players get updated neural network
            if has_ai_player:
                for player_id, player in self.players.items():
                    if isinstance(player, MCTSComputerPlayer):
    ...
    def _force_process_log_buffer(self):
        """Force immediate processing of log buffer"""
        try:
            messages = []
            with self.log_buffer_lock:
                if self.log_buffer:
    ...
    def generate_tactical_position(self):
        """Generate a tactical position where a specific response is needed"""
        game = Connect4Game()
    ...
    def _training_worker_thread(self, n):
        try:
            ...
            # Use the global function (not a class method)
            game_func = partial(
                _play_single_training_game,  # Correct reference to the global function
            ...
            # New: Track number of tactical games injected
            tactical_games = 0
            tactical_injection_frequency = 20  # Inject tactical position every N games
            ...
            try:
                ...
                while games_completed < n and not self.training_stop_requested:
                    ...
                    # New: Inject tactical positions at regular intervals
                    if games_completed > 0 and games_completed % tactical_injection_frequency == 0:
                        ...
                        if best_move is not None:
                            ...
                            # Create a game from the tactical position
                            game_state = tactical_position
                            
                            # Create policy target that emphasizes the correct move
                            policy = np.zeros(COLUMN_COUNT, dtype=np.float32)
                            
                            if position_type == "win":
                                # For winning moves, put 90% probability on best move
                                policy[best_move] = 0.9
                                # Distribute remaining 10% among valid moves
                                valid_moves = game_state.valid_moves()
                                if len(valid_moves) > 1:
                                    for m in valid_moves:
                                        if m != best_move:
                                            policy[m] = 0.1 / (len(valid_moves) - 1)
                                value = 1.2  # Strongly positive value
                            else:  # "block"
                                # For blocking moves, put 80% probability on best move
                                policy[best_move] = 0.8
                                # Distribute remaining 20% among valid moves
                                valid_moves = game_state.valid_moves()
                                if len(valid_moves) > 1:
                                    for m in valid_moves:
                                        if m != best_move:
                                            policy[m] = 0.2 / (len(valid_moves) - 1)
                                value = 0.8  # Moderately positive value
                            
                            # Add the tactical position directly to the training data
                            example_data = {
                            ...
                            # Add example to our training data
                            all_examples.append([example_data])
                            all_game_results.append(game_state.current_player)  # Marking as a win
