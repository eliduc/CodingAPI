...
Version 8.1 - UI improvements and additional training metrics
...
class MCTS:
    ...
    def search(self, root_state: 'Connect4Game'):
        ...
        for i, m in enumerate(valid):
            # Only mix with Dirichlet noise if explore is True
            if self.explore:
                p = 0.75 * prior[m] + 0.25 * dirichlet[i]
            else:
                # When in full strength mode (explore=False), use pure policy from NN
                p = prior[m]
...
class MCTSComputerPlayer(Player):
    def __init__(self, iters, c, nn: NNManager, explore=True, model_path=None):
        ...
        # When explore=False (Play @ Full Strength), set c_puct to 0.0 to exploit more
        self.mcts = MCTS(iters, c if explore else 0.0, nn, explore=explore)
    ...
    def get_move(self, state, gui=None):
        ...
        for move in valid_moves:
            ...
            if test_state.game_over and test_state.winner == state.current_player:
                # If it's a winning move, create training example (one-hot) and return immediately
        ...
        for move in valid_moves:
            ...
            for opp_move in test_state.valid_moves():
                ...
                if test_state2.game_over and test_state2.winner == opponent:
                    # Found a move that blocks an immediate opponent win
                    # Create training example for this blocking move
                    vp = np.zeros(COLUMN_COUNT, dtype=np.float32)
                    vp[move] = 1.0
                    self.nn.add_example(state, vp)
                    return move
...
class PlayerDialog(simpledialog.Dialog):
    def __init__(self, master, mcts_params, nn):
        ...
        self.continuous_play = tk.BooleanVar(master, False)     # Add continuous play option
    ...
    def body(self, m):
        ...
        # Add Continuous Play checkbox in a new row
        self.continuous_check = ttk.Checkbutton(m, text="Continuous Play", variable=self.continuous_play)
        self.continuous_check.grid(row=2, column=0, columnspan=6, sticky="w", padx=5, pady=(10,5))
...
class SettingsDialog(simpledialog.Dialog):
    def __init__(self, master, mcts_params, train_games=200, max_cc_games=100, nn_params=None):
        ...
        self.max_cc = tk.StringVar(master, str(max_cc_games))  # Add max_cc_games parameter
    ...
    def _add_training_presets(self, train_tab):
        ...
        preset_frame.grid(row=2, column=0, columnspan=2, sticky="ew", padx=5, pady=10)
    ...
    def validate(self):
        try:
            ...
            max_cc = int(self.max_cc.get())  # Validate max_cc_games
            ...
            if (it <= 0 or c < 0 or games <= 0 or max_cc <= 0 or lr <= 0 or  # Include max_cc check
    ...
    def apply(self):
        self.result = {
            ...
            'training': {
                'games': int(self.games.get()),
                'max_cc_games': int(self.max_cc.get())  # Include max_cc_games in result
            },
            'nn_params': {
...
class Connect4GUI(tk.Tk):
    def __init__(self):
        ...
        self.fullplay_check = ttk.Checkbutton(control_options, text="Play @ Full Strength", variable=self.fullplay_var)
        self.fullplay_check.grid(row=0, column=1, sticky="w", padx=5)
        self.fullplay_check.state(['disabled'])  # Disabled until players are selected
        ...
        train_frame.grid(row=0, column=2, padx=5, sticky="e")
    ...
    def _new_game(self):
        ...
        if self.is_comp and str(self.train_frame.grid_info()) != "{}":
            ...
            self.train_log.insert("end", f"Game #{game_number}\n")
            self.train_log.insert("end", f"{'-'*20}\n")
    ...
    # Choose players dialog - now with continuous play option
    def _choose_players(self):
        ...
        if self.is_comp:
            ...
            self.fullplay_check.state(['!disabled'])  # Enable fullplay
        else:
            ...
            if has_ai:
                ...
                self.fullplay_check.state(['disabled'])  # Disable fullplay
        ...
        if has_ai:
            ...
            if self.is_comp:
                self.train_log.insert("end", "Computer (AI) vs Computer (AI) Game\n")
            else:
                self.train_log.insert("end", "Human vs Computer (AI) Game\n")
                
            self.train_log.insert("end", "-" * 50 + "\n")
            ...
            self.train_log.insert("end", f"Red Model: {red_model}\n")
            self.train_log.insert("end", f"Green Model: {green_model}\n")
            self.train_log.insert("end", f"MCTS iterations: {self.mcts_params['iterations']}\n")
            self.train_log.insert("end", f"Exploration: {'Disabled' if self.fullplay_var.get() else 'Enabled'}\n")
            
            if self.is_comp:
                self.train_log.insert("end", f"Continuous Play: {'Enabled' if self.continuous_play else 'Disabled'}\n")
                self.train_log.insert("end", f"Max games: {self.max_cc_games}\n")
    ...
    def _make_move(self, col):
        ...
        if (self.nn and self.learn_var.get() and 
            ...
            not isinstance(self.players[self.game.current_player], RandomComputerPlayer)):  # Added check for RandomComputerPlayer
    ...
    def _finish(self):
        ...
        if self.is_comp and str(self.train_frame.grid_info()) != "{}":
            ...
            if self.game.winner == 'Draw':
                self.train_log.insert("end", "Result: Draw\n")
            else:
                winner_name = "Red" if self.game.winner == RED_PIECE else "Green"
                self.train_log.insert("end", f"Result: {winner_name} wins\n")
            self.train_log.insert("end", f"Current score - Red: {self.score['red']}, Green: {self.score['green']}, Draws: {self.score['draws']}\n")
            self.train_log.config(state="disabled")
            self.train_log.see("end")
        ...
        if (not has_random_player) and (self.is_comp or self.learn_var.get()):
            ...
            if self.is_comp and str(self.train_frame.grid_info()) != "{}":
                ...
                self.train_log.config(state="normal")
                self.train_log.insert("end", "Training neural network...\n")
                self.train_log.config(state="disabled")
                self.train_log.see("end")
            ...
            if self.is_comp:
                ...
                if str(self.train_frame.grid_info()) != "{}":
                    self.train_log.config(state="normal")
                    self.train_log.insert("end", f"Collected {len(self.nn.pending)} training examples\n")
                    self.train_log.config(state="disabled")
                    self.train_log.see("end")
            
            self.nn.finish_game(self.game.winner)
            
            # Pass the number of games being trained on (1 for a single game)
            self.nn.train(logger=self.log_to_training, num_games=1)
            ...
            if has_random_player and self.is_comp and str(self.train_frame.grid_info()) != "{}":
                self.train_log.config(state="normal")
                self.train_log.insert("end", "Training skipped - Random Computer player present\n")
                self.train_log.config(state="disabled")
                self.train_log.see("end")
            
        # Check if we should continue with another game
        if self.is_comp and not self.paused:
            ...
            if self.continuous_play and self.score['games'] < self.max_cc_games:
                ...
                if self.continuous_play and self.score['games'] >= self.max_cc_games:
                    self.train_log.config(state="normal")
                    self.train_log.insert("end", f"Maximum number of games ({self.max_cc_games}) reached. Stopping continuous play.\n")
                    self.train_log.config(state="disabled")
                    self.train_log.see("end")
    ...
    def _settings(self):
        ...
        original_settings = {
            ...
            'training': {'games': self.train_games, 'max_cc_games': self.max_cc_games},
            'nn_params': dict(self.nn_params)
        }
        
        dlg = SettingsDialog(self, self.mcts_params, self.train_games, self.max_cc_games, self.nn_params)
        if dlg.result:
            ...
            # Check max CC games
            if new_max_cc_games != self.max_cc_games:
                settings_changed = True
    ...
    def _train(self):
        ...
        self.train_log.insert("end", f"Starting training with {n} self-play games\n")
        self.train_log.insert("end", f"Model: {os.path.basename(self.training_model_path)}\n")
        self.train_log.insert("end", f"MCTS iterations: {self.mcts_params['iterations']}\n")
        self.train_log.insert("end", f"Exploration: {'Disabled' if self.fullplay_var.get() else 'Enabled'}\n")
        self.train_log.insert("end", f"Batch size: {self.nn_params['batch_size']}, Epochs: {self.nn_params['epochs']}\n")
        self.train_log.insert("end", "-" * 50 + "\n\n")
        self.train_log.config(state="disabled")
        self.train_log.see("end")
    ...
    def _training_worker_thread(self, n):
        ...
        try:
            ...
            # Important: use self.fullplay_var.get() to correctly respect Play @ Full Strength setting
            game_func = partial(
                _play_single_training_game, 
                self.mcts_params['iterations'], 
                self.mcts_params['C_param'], 
                self.nn,
                not self.fullplay_var.get()  # exploration param - respects Play @ Full Strength setting
            )
            ...
            if games_completed > 0:
                ...
                if not hasattr(self, 'force_stop_training') or not self.force_stop_training:
                    ...
                    # Train the neural network with our logger and pass number of games
                    self.nn.train(
                        batch_size=self.nn_params['batch_size'], 
                        epochs=self.nn_params['epochs'], 
                        start_time=start_time,
                        logger=self.log_to_training,
                        num_games=games_completed  # Pass the actual number of games completed
                    )
                    ...
                    # Update model info display to show new total games
    ...
    # Fixed pause method with updated behavior for continuous play stopping
    def _pause(self):
        ...
        if self.is_comp and self.game_in_progress:
            # First, cancel any scheduled auto move or job to prevent pending moves
            if self.auto_job:
                self.after_cancel(self.auto_job)
                self.auto_job = None
                
            # Set paused flag FIRST to prevent any new moves from being processed
            self.paused = True
            
            # Cancel ALL pending "after" events more thoroughly
            ...
            # If this is a continuous play, show confirmation dialog
            if self.continuous_play:
                dialog = StopConfirmationDialog(self, mode="continuous")
                if dialog.result is None:  # User clicked Continue or closed dialog
                    # Re-enable game if user wants to continue
                    self.paused = False
                    self.after(30, self._next_turn)
                    return
            
            # User confirmed stop or it's not continuous play
            self.stop_btn['state'] = "disabled"
            self.game_in_progress = False  # Add this line to fully stop game logic
            
            # Now reset the game and clear the history
            self.game.reset()
            self.moves.config(state="normal")
            self.moves.delete("1.0", "end")
            self.moves.config(state="disabled")
            ...
            if self.continuous_play:
                self._set_status("Continuous play stopped")
