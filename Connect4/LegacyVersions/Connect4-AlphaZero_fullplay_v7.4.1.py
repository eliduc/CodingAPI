...
class MCTS:
    ...
    def search(self, root_state: 'Connect4Game'):
        ...
        # Then check if we need to block an upcoming opponent win (original logic)
        for move in valid_moves:
            test_state = root_state.copy()
            test_state.drop_piece(move)
            test_state.switch()
...
def _play_single_training_game(mcts_iterations, puct_c, nn_manager_config):
    """
    Self-contained function to play a single training game and return states, 
    policies (full visit distributions), and game result.
    """
    ...
    while not game.game_over:
        ...
        # Call MCTS search to get the move and collect visit distribution
        state_before = game.copy()
        
        # Get move from MCTS - search already adds the example with visit probs to nn_copy
        move = player_ai.get_move(game)
        
        # Get the most recent training example (which contains visit probs)
        if nn_copy.pending:
            # Save the state and visit probs
            training_example = {
                'state': state_before,
                'policy': nn_copy.pending[-1]['policy'].numpy(),
                'player': player
            }
    ...
    # Return the training examples and the game result
    return training_examples, game.winner
...
class Connect4GUI(tk.Tk):
    ...
    def _choose_players(self):
        ...
        if has_ai:
            ...
            self.train_log.insert("end", f"Exploration: {'Disabled' if dlg.result['full_strength'] else 'Enabled'}\n")
    ...
    def _training_worker_thread(self, n):
        ...
        try:
            ...
            # FIX 2/3: Use the fixed _play_single_training_game function that returns full visit distributions
            # Prepare NN manager config to pass to workers (we can't pass the object directly)
            nn_config = {
                ...
                while games_completed < n and not self.training_stop_requested:
                    ...
                    while pending and not self.training_stop_requested:
                        ...
                        for future in done:
                            ...
                            try:
                                # FIX 2/3: Get training examples with full visit distributions
            ...
            if games_completed > 0:
                ...
                # FIX 2/3: Use full visit distributions returned from workers
                for i, (examples, winner) in enumerate(zip(all_training_examples, all_game_results)):
                    ...
                    # Process this game's examples
                    for example in examples:
                        state = example['state']
                        policy = example['policy']
                        
                        # Add example to training data using the full visit distribution
                        self.nn.add_example(state, policy)
                    
                    # Finalize the game with correct winner
                    self.nn.finish_game(winner)
                    
                    # Log a progress indicator for large datasets
                    if i % 10 == 0 and i > 0:
