...
Version a12.5 - Enhanced training with decision-focused learning and persistent logs
    ...
    def finish_game(self, winner):
        ...
        for ex in self.pending:
            if winner == 'Draw':
                ...
                # Increase win value from 1.0 to 1.2 to increase its training signal
                value = 1.2  # Modified: Increased from 1.0 to emphasize wins
                win_count += 1
            else:
                # Increase loss magnitude from -1.0 to -1.2 to emphasize losses
                value = -1.2  # Modified: Changed from -1.0 to emphasize losses
                loss_count += 1
            ...
            # Give higher priority to losing positions and non-draw positions
            if value < 0:
                surprise *= 2.0  # Double the priority for losing positions
            
            # Modified: Give additional priority to non-draw positions
            if winner != 'Draw':
        ...
        if len(self.data['states']) > MAX_TRAINING_EXAMPLES:
            ...
            if hasattr(self, 'data') and 'priorities' in self.data and self.data['priorities']:
                ...
                # Modified: Bias keeping non-draw examples
...
class MCTS:
    ...
    def search(self, root_state: 'Connect4Game', previous_root=None, move_number=0):
        ...
        # Use a 5% chance to bypass the safe move logic when in explore mode
        bypass_safe_logic = self.explore and random.random() < 0.05
        ...
        for m in valid:
            biased_priors[m] = prior[m] * CENTER_BIAS[m]
...
class MCTSComputerPlayer(Player):
    def __init__(self, iters, c, nn: NNManager, explore=True, model_path=None, dirichlet_noise=0.3):
        ...
        self.mcts = MCTS(iters, c if explore else 0.8, nn, explore=explore, dirichlet_noise=dirichlet_noise)
