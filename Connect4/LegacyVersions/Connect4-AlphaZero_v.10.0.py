...
class Connect4ResNet(nn.Module):
    def __init__(self, channels=128, num_res_blocks=15):  # Increased from 10 to 15 residual blocks
        super().__init__()
        # Initial processing
        ...
        # Policy head
        self.conv_pol = nn.Conv2d(channels, 64, kernel_size=1)  # Increased from 32 to 64 channels
        self.bn_pol = nn.BatchNorm2d(64)
        self.fc_pol = nn.Linear(64 * ROW_COUNT * COLUMN_COUNT, COLUMN_COUNT)
        
        # Value head
        self.conv_val = nn.Conv2d(channels, 64, kernel_size=1)  # Increased from 32 to 64 channels
...
class SettingsDialog(simpledialog.Dialog):
    ...
    def body(self, m):
        ...
        self.dirichlet_noise = tk.StringVar(self.master, "0.3")  # Default value
...
class Connect4GUI(tk.Tk):
    ...
    def _training_worker_thread(self, n):
        try:
            ...
            # Use the global function instead of the class method
            game_func = partial(
                MCTSComputerPlayer._play_single_training_game,  # <-- Corrected reference
