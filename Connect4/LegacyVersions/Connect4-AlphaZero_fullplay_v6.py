    ...
    def train(self, batch_size=None, epochs=None):
        ...
        for ep in range(epochs):
            ...
            for s, p_target, v_target in dl:
                ...
                # Apply weights to balance training
...
class Connect4GUI(tk.Tk):
    ...
    # ENHANCED: Training with progress display in main window
    def _train(self):
        if self.game_in_progress and not self.paused:
        ...
        def worker():
            ...
            # Create AI players with the selected model for training
            ...
            # For tracking separate model updates
            red_nn = None
            green_nn = None
            ...
            while i < n and not self.training_stop_requested:
                g = Connect4Game()
                cur = RED_PIECE
                game_moves = []  # Store moves for later training
                ...
                if not self.training_stop_requested:
                    ...
                    for move in game_moves:
                        ...
                        # Add example to our primary training model
                        self.nn.add_example(state, vp)
                    
                    # Now finish the game for our main model
                    self.nn.finish_game(g.winner)
                    
                    # If we have separate models for red and green, train them too
                    # but only if they're not the same as our primary model
                    if not using_same_model:
                        ...
                        if green_nn and green_model_path != self.training_model_path:
                            ...
                            green_nn.finish_game(g.winner)
                    
                    i += 1
                    # Update progress in main window
                    self.after(0, lambda idx=i: self._update_training_progress(idx, n))
            ...
            if i > 0:
                ...
                # Train the player models if needed
                if not using_same_model:
                    if red_nn and red_model_path != self.training_model_path:
            ...
            self.after(0, lambda: self.train_browse.config(state="normal"))  # Re-enable model selection
            self.after(0, lambda: setattr(self, 'training_in_progress', False))
            
            # Update training progress area when done
            if self.training_stop_requested:
                self.after(0, lambda: self.train_status.config(text=f"Stopped after {i} games"))
                messagebox.showinfo("Stopped", f"Training stopped after {i} games.")
            else:
                self.after(0, lambda: self.train_status.config(text=f"Completed {n} games"))
