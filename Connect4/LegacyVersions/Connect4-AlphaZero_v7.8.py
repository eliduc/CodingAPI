...
class SettingsDialog(simpledialog.Dialog):
    ...
    def body(self, m):
        ...
        # MODIFIED: Renamed field from "C-C games before training" to "AI-AI games before training"
        ttk.Label(train_tab, text="AI-AI games before training:").grid(row=2, column=0, sticky="w", padx=5, pady=5)
        ...
        # MODIFIED: Renamed field from "Delay in C-C games (ms)" to "Delay in AI-AI games (ms)"
        ttk.Label(train_tab, text="Delay in AI-AI games (ms):").grid(row=3, column=0, sticky="w", padx=5, pady=5)
...
class Connect4GUI(tk.Tk):
        ...
        if self.is_comp and not self.paused:
            ...
            if self.continuous_play and self.score['games'] < self.max_cc_games:
                ...
                if self.continuous_play and self.score['games'] >= self.max_cc_games:
                    ...
                    # Set status to Ready to play
                    self.after(1000, lambda: self._set_status("Ready to play..."))
        else:
            # If training is complete and not playing AI vs AI, set status to Ready to play
            if not self.continuous_play and not self.is_comp:
    ...
    def _train(self):
        ...
        # MODIFIED: Update main status area to show training is in progress
        self._set_status("NN is training...", "#990000")  # Dark red color
    ...
    def _training_worker_thread(self, n):
        try:
            ...
            # ADDED: Reset the status text after training completes
            ...
            # ADDED: Reset the status text after error
            self.after(0, lambda: self._set_status("Ready to play..."))
