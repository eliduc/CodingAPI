    ...
    def _training_worker_thread(self, n):
        try:
                    ...
                    while pending and not self.training_stop_requested:
                        ...
                        for future in done:
                            ...
                            try:
                                ...
                                # Use local variables for lambda
                                progress = games_completed
                                time_str = elapsed_str
                                
                                # Update UI thread
                                self.after(0, lambda p=progress, t=time_str: 
                                ...
                                if games_completed % max(1, n // 10) == 0 or games_completed % 5 == 0:
                                    ...
                                    red_wins = sum(1 for w in all_results if w == RED_PIECE)
                                    green_wins = sum(1 for w in all_results if w == GREEN_PIECE)
                                    draws = sum(1 for w in all_results if w == 'Draw')
            ...
            if games_completed > 0:
                ...
                for i, (example_data, winner) in enumerate(zip(all_examples, all_results)):
                    ...
                    try:
                        ...
                        # Only finish_game at the end of each game sequence
                        if i == len(all_examples) - 1 or (i < len(all_results) - 1 and all_results[i] != all_results[i+1]):
                            self.nn.finish_game(winner)
                        
                        # Log progress for large datasets
                        if i % 10 == 0 and i > 0:
                            self.log_to_training(f"Processed {i}/{len(all_results)} games")
                            
                    except Exception as proc_error:
                        self.log_to_training(f"Error processing example {i}: {str(proc_error)}")
