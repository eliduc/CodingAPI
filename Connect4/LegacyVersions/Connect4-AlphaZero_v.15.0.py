...
class TreeNode:
    ...
    def __init__(self, state: 'Connect4Game', parent=None, move=None, prior=0.0, original_prior=None, nn_version=None):
        ...
        self.nn_version = nn_version  # Now properly initialized
...
class MCTS:
    ...
    def search(self, root_state: 'Connect4Game', previous_root=None, move_number=0):
        ...
        self.root.nn_version = getattr(self.nn, 'version', None)  # Fix for issue #1
        ...
        # Create child nodes with nn_version set
        for m in valid:
    ...
    def _update_visualization(self, force_game_over=False):
        try:
            ...
            if self.game.game_over or force_game_over:
                ...
                # Show win probabilities as percentages (Issue #10 fix)
                ax.text(
                    1.5, -0.1,
                    f"Red: {red_win_prob:.0%}",
                    ha='center', va='center',
            ...
            if hasattr(self, 'nn'):
                ...
                # Convert to win percentages for clearer UI (Issue #10 fix)
                ...
                # Show value as win probability percentage (Issue #10 fix)
                current_player_win_prob = red_win_prob if current_state.current_player == RED_PIECE else green_win_prob
                value_text = f"Win Prob: {current_player_win_prob:.1%}"
                    
                ax.text(
                    4.0, -0.22 * max_height,  # Center position
                    value_text,
                    ha='center', va='center',
                ...
                # Add win probabilities as percentages (Issue #10 fix)
                ax.text(
                    1.5, -0.22 * max_height,
                    f"Red: {red_win_prob:.0%}",
                    ha='center', va='center',
