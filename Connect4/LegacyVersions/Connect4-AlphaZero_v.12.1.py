...
Version 12.0 - Improved implementation with audit suggestions
...
class MCTS:
    ...
    def search(self, root_state: 'Connect4Game', previous_root=None, move_number=0):
        ...
        if safe_moves:
            # IMPROVED: Rather than random selection, evaluate safe moves with the neural network
            ...
            if len(safe_moves) > 1:
                ...
                for move in safe_moves:
                    ...
                    # Consider both value and policy in selection
                    move_score = value + 0.1 * base_policy[move] * CENTER_BIAS[move]
    ...
    def _normalize_reused_root(self):
        """
        Fix for tree-reuse bias: Scale down the visit count of a reused root node
        to prevent overweighting of exploration in children nodes.
        """
        ...
        # If there's a significant imbalance between root visits and child visits
        if self.root.visits > 0 and child_visit_sum / self.root.visits < 0.5:
            # Scale factor: We want child visits to be approximately proportional
            # to their real visit percentages, but with reduced absolute values
            scale_factor = 0.5
            
            # Get maximum child visit count
            max_child_visits = max(child.visits for child in self.root.children.values()) if self.root.children else 0
            
            if max_child_visits > 0:
                # Scale down the root's visit count to a value that maintains reasonable exploration
                # but doesn't overweight the priors
                target_root_visits = max_child_visits / scale_factor
                
                # Ensure root visits don't go below a minimum threshold (e.g., 20)
                self.root.visits = max(20, min(self.root.visits, target_root_visits))
...
class MCTSComputerPlayer(Player):
    def __init__(self, iters, c, nn: NNManager, explore=True, model_path=None, dirichlet_noise=0.3):
        self.nn = nn
        self.model_path = model_path
        # FIXED: Increased c_puct for full-strength mode from 0.3 to 0.6
        self.mcts = MCTS(iters, c if explore else 0.6, nn, explore=explore, dirichlet_noise=dirichlet_noise)
