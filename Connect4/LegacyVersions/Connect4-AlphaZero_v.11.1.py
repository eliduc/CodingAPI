...
Version 11.1 - Improved implementation with audit suggestions
...
class Connect4GUI(tk.Tk):
    ...
    def __init__(self):
        ...
        # Create matplotlib figure and canvas with adjusted dimensions - INCREASED HEIGHT
        self.policy_fig = plt.Figure(figsize=(8, 6), dpi=100)  # Taller figure
    ...
    def _update_visualization(self):
        ...
        try:
            ...
            if hasattr(self, 'nn'):
                ...
                bars = ax.bar(bar_positions, probs, color='red', width=0.6)
                ...
                # Set axes properties
                ax.set_ylim(0.0, max(0.2, max(probs) * 1.2))
                ...
                # Increase title size and add more space at the top
                ax.set_title(f"{'Red' if current_state.current_player == RED_PIECE else 'Green'}'s Move Distribution", 
                          fontsize=16, color='red' if current_state.current_player == RED_PIECE else 'green',
                          pad=15)  # Add more padding above title
                
                # Add grid lines but make them less prominent
                ax.grid(True, linestyle='--', alpha=0.5, axis='y')
                
                # Remove bottom spines to save space
                ax.spines['bottom'].set_visible(False)
                
                # Create a text area for probability and value info right above X axis
                # Calculate Y position - just above the X-axis
                y_pos = -0.05
                
                # Value text in the center, right above X-axis
                ...
                # Add probabilities in a row above the value text
                ...
                # Add extra space at the bottom to fit our probabilities
                self.policy_fig.subplots_adjust(bottom=0.15)
    ...
    def _new_game(self):
        ...
        # Update policy visualization if window is open - SAFELY
        try:
            if hasattr(self, 'visualization_window') and self.visualization_window is not None:
                try:
                    if self.visualization_window.winfo_exists():
                        # Schedule visualization update after a short delay
                        self.after(100, self._update_visualization)
                except Exception as e:
                    print(f"Error checking visualization window: {e}")
                    # Reset visualization window if there was an error
                    self.visualization_window = None
                    self.policy_fig = None
                    self.policy_canvas = None
        except Exception as e:
            print(f"Error updating visualization during new game: {e}")
    ...
    def _training_worker_thread(self, n):
        try:
            ...
            # Use the global function with proper reference 
            game_func = partial(
                _play_single_training_game,  # Reference the global function
