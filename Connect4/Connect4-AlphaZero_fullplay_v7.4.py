...
Version 8.1 - Performance and UI improvements
...
class MCTS:
    ...
    def search(self, root_state: 'Connect4Game'):
        ...
        for i, m in enumerate(valid):
            # Apply center-column bias to priors
            if self.explore:
                # Mix with Dirichlet noise and apply center bias
                p = (0.75 * prior[m] * CENTER_BIAS[m] + 0.25 * dirichlet[i])
            else:
                # When in full strength mode (explore=False), use pure policy from NN with center bias
                p = prior[m] * CENTER_BIAS[m]
            ...
            if not node.state.game_over:
                ...
                for m in valid:
                    if m not in node.children:
                        ...
                        if not ns.game_over: ns.switch()
                        # Apply center bias to new child nodes too
                        node.children[m] = TreeNode(ns, parent=node, move=m, prior=probs[m] * CENTER_BIAS[m])
            else:
                # terminal node value
...
class MCTSComputerPlayer(Player):
    ...
    def get_move(self, state, gui=None):
        ...
        # IMPROVED: First check if the opponent can win immediately with their next move
        ...
        # Then check if we need to block an immediate opponent win (original logic)
        for move in valid_moves:
            test_state = state.copy()
            test_state.drop_piece(move)
            test_state.switch()
...
class Connect4GUI(tk.Tk):
    ...
    def _training_worker_thread(self, n):
        ...
        try:
            ...
            # Important: use exploration=True for training since we want diverse exploration
            game_func = partial(
