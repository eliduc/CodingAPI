...
Version 8.0 - UI improvements and bug fixes
...
class Connect4GUI(tk.Tk):
    def __init__(self):
        ...
        self.go_btn = ttk.Button(control_frame, text="Continue", state="disabled", command=self._resume)
    ...
    def _choose_players(self):
        ...
        if self.is_comp:
            ...
            # Show training UI for AI vs AI games
            self.separator.grid()
            ...
            # Reset training log for new AI vs AI session
            self.train_log.config(state="normal")
            self.train_log.delete("1.0", "end")
            
            # Add header about AI vs AI game
            ...
            # Hide training UI if it was visible and this is not an AI vs AI game
    ...
    def _finish(self):
        ...
        if (not has_random_player) and (self.is_comp or self.learn_var.get()):
            ...
            # Use the log_to_training method for AI vs AI games too
            self.nn.train(logger=self.log_to_training)
        else:
    ...
    # Updated pause/stop for both games and training with responsive stopping
    def _pause(self):
        if self.training_in_progress:
        ...
        if self.auto_job:
            self.after_cancel(self.auto_job)
            self.auto_job = None
        self._set_status("Game stopped")  # Changed from "Match paused" to "Game stopped"
