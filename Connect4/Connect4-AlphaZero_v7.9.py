...
class SettingsDialog(simpledialog.Dialog):
    def __init__(self, master, mcts_params, train_games=200, max_cc_games=100, cc_train_interval=50, 
        ...
        # FIXED: Ensure games_before_training is a number, not a dictionary
    ...
    def body(self, m):
        ...
        # Training tab - FIXED fields and layout
        train_tab = ttk.Frame(nb)
        nb.add(train_tab, text="Training")
        
        # Games in Train NN mode field
        ...
        # FIXED: AI-AI games before training field (was using wrong row number)
        ...
        # FIXED: Delay in AI-AI games field (was using wrong row number)
        ...
        # Add training presets to the training tab - FIXED: Moved to row 6 to avoid conflict
    ...
    def _add_training_presets(self, train_tab):
        ...
        # FIXED: Training presets frame - use row 6 to avoid conflicts with fields
    ...
    def validate(self):
        try:
            ...
            # ADDED: Check that games_before_training is not greater than games
            ...
            # ADDED: Check that AI-AI games before training is not greater than games in AI-AI mode
    ...
    def apply(self):
        self.result = {
            ...
            'training': {
                'games': int(self.games.get()),
                'games_before_training': int(self.games_before_training.get()),  # New field
...
class Connect4GUI(tk.Tk):
    def __init__(self):
        ...
        # FIXED: Initialize games_before_training based on train_games
        ...
        self.status_blink_job = None  # NEW: For blinking status messages
        ...
        # Initialize training state flags
        ...
        # FIXED: Increase canvas height to accommodate text under the board
        ...
        # Create a frame for score and reset button
        score_frame = ttk.Frame(side)
        score_frame.grid(row=1, column=0, columnspan=3, sticky="ew", pady=(0,10))
        score_frame.grid_columnconfigure(0, weight=1)  # Score label expands
        
        # Score label (now inside score_frame)
        ...
        # NEW: Reset Score button (at the right of score_frame)
        ...
        # Train NN button and model selector - MODIFIED with new format
        ...
        # Control buttons: New Game, Select Players, Stop, Exit
        ...
        exit_btn.pack(side="right", padx=5)  # Use side="right" to place it on the right
        ...
        self._draw()
        self._update_score()
        self._set_status("Ready to play...")  # Set default status
    ...
    def _reset_score(self):
        ...
        # Add entry to training log if visible
    ...
    def _new_game(self):
        ...
        # MODIFIED: Updated status message format with trailing ellipsis
    ...
    def _choose_players(self):
        ...
        # MODIFIED: Update status message based on player type
        if isinstance(self.players[self.game.current_player], HumanPlayer):
            self._set_status(f"{PLAYER_MAP[self.game.current_player]}'s turn...", COLOR_MAP[self.game.current_player])
        else:
            self._set_status(f"{PLAYER_MAP[self.game.current_player]} is thinking...", COLOR_MAP[self.game.current_player])
    ...
    # Modified _draw method to display rivals under the board
    def _draw(self):
        ...
        # Create a blue background rectangle for text area below the board
        text_bg_y = HEIGHT + 5
        text_bg_height = 80  # Make it taller to ensure visibility
        ...
        if hasattr(self, 'training_in_progress') and self.training_in_progress:
            ...
            # Regular human vs AI or human vs human play
            # Get player types
    ...
    def _blink_status(self, msg, color):
        ...
        if current_color == color:
            self.status.config(foreground="#777777")  # Dim color when blinking
        else:
            self.status.config(foreground=color)  # Normal color
        
        # Continue blinking if the status hasn't changed
        if self.status.cget("text") == msg:
            self.status_blink_job = self.after(500, lambda: self._blink_status(msg, color))

            
    def _update_score(self):
        # MODIFIED: Added colons to the score labels
    ...
    def _make_move(self, col):
        ...
        if self.game.game_over:
            ...
            if not self.play_till_end:
                # Update with appropriate message based on player type
                if isinstance(self.players[self.game.current_player], HumanPlayer):
                    self._set_status(f"{PLAYER_MAP[self.game.current_player]}'s turn...", COLOR_MAP[self.game.current_player])
                else:
                    self._set_status(f"{PLAYER_MAP[self.game.current_player]} is thinking...", COLOR_MAP[self.game.current_player])
    ...
    # Changed from standalone function to a method of Connect4GUI
    def _process_collected_games(self, all_examples, all_game_results, start_game, end_game, elapsed_str):
        """Process a batch of collected games for training"""
        try:
            ...
            if not hasattr(self, 'force_stop_training') or not self.force_stop_training:
                ...
                # Set nn_training_phase flag to True and update UI
                self.nn_training_phase = True
                # Force an immediate redraw to update the display with "Training on self-played games..."
                ...
                self.nn.train(
                    ...
                    start_time=None,  # We're already tracking overall time
                ...
                # Reset nn_training_phase flag to False and update UI
                self.nn_training_phase = False
                # Force an immediate redraw to update the display back to "Self-play mode..."
                self.after(0, lambda: self._draw())
                self.update_idletasks()  # Force immediate update
            ...
            self.nn_training_phase = False
            self.after(0, lambda: self._draw())  # Update the display
            return False
    ...
    def _finish(self):
        ...
        if self.play_till_end:
            ...
            # Set status to "Ready to play" after finishing with play_till_end
            self.after(1200, lambda: self._set_status("Ready to play..."))
            
            # Don't start a new game after training when stopped
            return
        
        # MODIFIED: Training for both AI vs AI games and Human vs AI with Learn@Play
        # AI vs AI training interval check
        if self.is_comp and should_train and not self.paused:
            ...
            # Schedule training to happen after UI has been refreshed
            self.after(50, self._perform_end_game_training)
        # Human vs AI with Learn@Play training check
        elif not self.is_comp and self.learn_var.get() and not self.paused:
            # For Human vs AI, train after every game if Learn@Play is enabled
            has_ai = any(isinstance(p, MCTSComputerPlayer) for p in self.players.values())
            if has_ai:
                if str(self.train_frame.grid_info()) != "{}":
            ...
            # No training needed yet for AI vs AI
            ...
            if self.continuous_play and self.score['games'] < self.max_cc_games:
                ...
                if self.continuous_play and self.score['games'] >= self.max_cc_games:
                    ...
                    if self.games_since_training > 0:
                        ...
                        # Show message box immediately if no training needed
                        self.after(500, lambda: messagebox.showinfo("Continuous Play Complete", 
                                                  f"Maximum number of games ({self.max_cc_games}) reached.\n\n"
                                                  f"Final score:\nRed: {self.score['red']}\nGreen: {self.score['green']}\nDraws: {self.score['draws']}"))
                    
                    # MODIFIED: Set status to "Ready to play" after continuous play ends
                    self.after(1200, lambda: self._set_status("Ready to play..."))
        else:
            # For single games, leave the status as is (Draw/Red wins/Green wins)
            pass
    ...
    def _perform_end_game_training(self):
        ...
        if (not has_random_player) and (self.is_comp or self.learn_var.get()):
            # MODIFIED: Update main status to show training is in progress
            self._set_status("NN is training...", "#990000")  # Dark red color
            
            # Set performing_training flag to True
            self.performing_training = True
            self._draw()  # Update the display
            ...
            # Reset performing_training flag
            self.performing_training = False
            self._draw()  # Update the display
            
            # MODIFIED: Reset main status after training is complete with appropriate message
            if self.game.game_over:
                if self.game.winner == 'Draw':
                    self._set_status("Draw")
                else:
    ...
    def _settings(self):
        ...
        original_settings = {
            ...
            'training': {
                ...
                'games_before_training': self.games_before_training,  # Make sure this is included!
        ...
        if dlg.result:
            ...
            new_games_before_training = dlg.result['training']['games_before_training']  # Make sure this is retrieved!
            ...
            if (new_train_games != self.train_games or
                new_games_before_training != self.games_before_training or  # Check this value too!
            ...
            self.games_before_training = new_games_before_training  # Update this value!
            ...
            cfg = {
                ...
                'training': {
                    'games': self.train_games, 
                    'games_before_training': self.games_before_training,  # Save this value!
                    'max_cc_games': self.max_cc_games,
                    'cc_train_interval': self.cc_train_interval,
                    'cc_delay': self.cc_delay
                },
    ...
    def _train(self):
        ...
        # Initialize training flags
        self.nn_training_phase = False
        self.performing_training = False
        self.training_in_progress = True
        
        # Force immediate redraw to show "Self-play mode..."
        self._draw()
        ...
        # MODIFIED: Update main status area to show "Training NN..."
        ...
        self.log_to_training(f"Training after every {self.games_before_training} games")  # NEW
    ...
    # New method to update results at the top of the window during training
    def _update_status_during_training(self, current, total, results):
        """Update the status area at the top of the window during training"""
        # Set only "Training NN..." in the status bar
        self._set_status("Training NN...", "#990000")  # Use dark red color for training status
        
        # Update the score label with the current game results
    ...
    def _training_worker_thread(self, n):
        try:
            # Initialize nn_training_phase flag
            self.nn_training_phase = False
            self.after(0, lambda: self._draw())  # Update the display
            self.update_idletasks()  # Force immediate update
            ...
            self.log_to_training(f"Batch size: {self.nn_params['batch_size']}, Epochs: {self.nn_params['epochs']}")
            self.log_to_training(f"Training after every {self.games_before_training} games")  # Make sure this uses the correct value
            self.log_to_training("-" * 50)
            ...
            # IMPORTANT: Use the correct games_before_training value
                    ...
                    while pending and not self.training_stop_requested:
                        ...
                        for future in done:
                            ...
                            try:
                                ...
                                # Update score label with game results
                                ...
                                if games_completed % max(1, n // 10) == 0 or games_completed % 5 == 0:
                                    ...
                                    # Log games since last training to show it's tracking correctly
                                    self.log_to_training(f"Games since last training: {games_since_last_training}/{self.games_before_training}")
                                
                                # IMPORTANT: Check if we should train after this batch of games
                                # Use the correct self.games_before_training value, not a hardcoded number
                                if games_since_last_training >= self.games_before_training and not self.training_stop_requested:
                                    ...
                                    # Process the collected games for training
                                    if self._process_collected_games(all_examples, all_game_results, 
                                                              start_game, end_game, elapsed_str):
                                        # After training, reset counter to 0 since we've just trained
                                        games_since_last_training = 0
            ...
            # Process remaining collected games and train models
            if games_completed > 0 and games_since_last_training > 0:
                ...
                # Calculate start game for the remaining games
                start_game = games_completed - games_since_last_training + 1
                end_game = games_completed
                
                # Process the remaining games for training
            ...
            # Reset score label to normal display
            ...
            # ALWAYS reset to "Ready to play" after training completes
            ...
            # Reset score label to normal display even on error
            self.after(0, lambda: self._update_score())
            ...
            # ALWAYS reset to "Ready to play" after training error
            self.after(0, lambda: self._set_status("Ready to play..."))
    ...
    def _pause(self):
        ...
        if self.is_comp and self.game_in_progress:
            ...
            # Update status - MODIFIED to use blinking status
                ...
                # Handle new format
                    ...
                    if 'training' in cfg:
                        ...
                        if 'games_before_training' in cfg['training']:
                            ...
                            # FIXED: If games_before_training is not in the config, set it based on train_games
                            self.games_before_training = min(100, self.train_games)
                    ...
                    # FIXED: Ensure games_before_training is not greater than train_games after loading
