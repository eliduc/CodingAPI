...
class Connect4GUI(tk.Tk):
    ...
    def _pause(self):
        ...
        if self.is_comp and self.game_in_progress:
            ...
            # Cancel specific game-related events rather than all events
            game_event_ids = []
            for after_id in self.tk.eval('after info').split():
                try:
                    if "next_turn" in self.after_info(int(after_id)) or "new_game" in self.after_info(int(after_id)):
                        game_event_ids.append(int(after_id))
                except (ValueError, KeyError):
                    pass
            
            # Cancel game-specific events
            for after_id in game_event_ids:
