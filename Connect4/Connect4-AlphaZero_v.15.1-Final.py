...
class Connect4GUI(tk.Tk):
    ...
    def _update_visualization(self, force_game_over=False):
        try:
            ...
            if self.game.game_over or force_game_over:
                ...
                # Store and show win probabilities
            ...
            if hasattr(self, 'players') and self.game.current_player in self.players:
                ...
                # Convert neural network value to win probabilities
                # The neural network returns a value from the current player's perspective
                current_player_win_prob = (value + 1) / 2.0  # Convert [-1,1] to [0,1]
                
                # Assign probabilities based on whose turn it is
                if current_state.current_player == RED_PIECE:
                    red_win_prob = current_player_win_prob
                    green_win_prob = 1.0 - red_win_prob
                else:
                    green_win_prob = current_player_win_prob
                    red_win_prob = 1.0 - green_win_prob
                ...
                # Create the bars
                bar_positions = np.arange(1, COLUMN_COUNT + 1)
                bars = ax.bar(bar_positions, probs, 
                             color='red' if current_state.current_player == RED_PIECE else 'green', 
                             width=0.6)
                ...
                # Show current player's win probability
                ...
                # Show absolute win probabilities for both players
                ax.text(
                    1.5, -0.22 * max_height,
                    f"Red: {red_win_prob:.0%}",
                    ha='center', va='center',
                ...
                # No players defined yet - show placeholder
                ax = self.policy_fig.add_subplot(111)
                ax.text(0.5, 0.5, "Select players to see visualization", 
                     ha='center', va='center', fontsize=12)
                ax.axis('off')
                self.policy_canvas.draw()
    ...
    def _update_win_probability(self):
        if not hasattr(self, 'players') or not self.game.current_player in self.players:
            return
            
        player = self.players[self.game.current_player]
        nn_manager = None
        
        # Determine which neural network to use
        if isinstance(player, MCTSComputerPlayer):
            nn_manager = player.nn
        else:
            # For human players or random players, use the main neural network if available
            nn_manager = self.nn
        ...
        if nn_manager:
            try:
                ...
                if self.game.game_over:
                    ...
                    # Update visualization on game over
                    if self.visualization_visible and hasattr(self, 'auto_update_var') and self.auto_update_var.get():
                        self._update_visualization(force_game_over=True)
                    
                    return  # Exit early after handling game over state
                else:
                    # Check for immediate win situations
                    ...
                    if can_win:
                        ...
                        # The neural network returns a value from the current player's perspective
                        # value = +1 means current player wins, -1 means current player loses
                        current_player_win_prob = (value + 1) / 2.0  # Convert [-1,1] to [0,1]
                ...
                self.current_green_win_prob = 0.5
                self.current_draw_prob = 0.0
                self.win_prob_var.set(f"Win Probs: Red: 50%/Green: 50%")
                self.win_prob_label.config(foreground="black")
        else:
            # No neural network available - show equal probabilities
            self.current_red_win_prob = 0.5
            self.current_green_win_prob = 0.5
            self.current_draw_prob = 0.0
            self.win_prob_var.set(f"Win Probs: Red: 50%/Green: 50%")
