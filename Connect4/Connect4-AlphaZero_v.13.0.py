...
Version a13.0 - Major algorithm improvements and bug fixes
...
class Connect4GUI(tk.Tk):
    ...
    def __init__(self):
        ...
        new_game_btn = ttk.Button(control_frame, text="New Game", style='Bold.TButton', command=self._new_game)
        new_game_btn.pack(side="left", padx=5)
        ToolTip(new_game_btn, "Start a new game with current players")
        
        select_players_btn = ttk.Button(control_frame, text="Select Players", style='Bold.TButton', command=self._choose_players)
        select_players_btn.pack(side="left", padx=5)
        ToolTip(select_players_btn, "Choose new players")
        
        self.stop_btn = ttk.Button(control_frame, text="Stop", style='Bold.TButton', state="disabled", command=self._pause)
        self.stop_btn.pack(side="left", padx=5)
        ToolTip(self.stop_btn, "Stop the current game, training, or continuous play")
        
        exit_btn = ttk.Button(control_frame, text="Exit", style='Bold.TButton', command=self.destroy)
    ...
    def _update_visualization(self):
        ...
        try:
            ...
            if hasattr(self, 'nn'):
                ...
                probs, value = self.nn.policy_value(current_state)
                        
                # Make sure we use the same values as in main win probability display
                if hasattr(self, 'current_red_win_prob') and hasattr(self, 'current_green_win_prob') and hasattr(self, 'current_draw_prob'):
                    red_win_prob = self.current_red_win_prob
                    green_win_prob = self.current_green_win_prob
                    draw_prob = self.current_draw_prob
                else:
                    # Calculate win probabilities if not already done
                ...
                # Create a new axis for the policy distribution - use the full figure space
                ax = self.policy_fig.add_subplot(111)
                ...
                # Create a text area for value info - with your updated positions
                ...
                # Add probabilities in a row at the bottom in a more visible position
    ...
    def _save_log_on_exit(self):
        ...
        # Create the complete log content from our persistent list
        complete_log_content = ""
        if hasattr(self, 'full_log_content'):
            complete_log_content = "".join(self.full_log_content)
        
        # If for some reason our full log is empty, try to get content from the GUI
        ...
        if complete_log_content:
            try:
                # Ensure we're writing in text mode with utf-8 encoding
    ...
    def _new_game(self):
        ...
        self.current_green_win_prob = 0.5
        self.current_draw_prob = 0.0
        self.win_prob_var.set(f"Win: {self.current_red_win_prob:.0%}/{self.current_draw_prob:.0%}/{self.current_green_win_prob:.0%}")
    ...
    def log_to_training(self, *args, **kwargs):
        ...
        # Force immediate update for important messages
        if "Training" in message or "Epoch" in message or "POLICY=" in message or "Value" in message:
            self.after(10, self._force_process_log_buffer)
                                
    def _update_log_from_buffer(self):
        current_time = time.time()
        if current_time - self.log_last_update > self.log_update_interval or self.log_buffer:
            messages = []
            with self.log_buffer_lock:
                if self.log_buffer:
    ...
    def _make_move(self, col):
        ...
        # FIXED: Ensure examples are added for human player when Learn@Play is enabled
        if self.learn_var.get():
            # Create one-hot vector for policy target
            vp = np.zeros(COLUMN_COUNT, dtype=np.float32)
            vp[col] = 1.0
            
            # Store example regardless of player type (as long as Learn@Play is on)
            if self.nn and not self.game.game_over:
                self.nn.add_example(state_before, vp)
                if str(self.train_frame.grid_info()) != "{}":
                    self.log_to_training(f"Added training example from human move (column {col+1})")
        ...
        if self.visualization_window is not None and self.visualization_window.winfo_exists() and self.auto_update_var.get():
            self._update_visualization()
    ...
    def _make_move(self, col):
        ...
        if self.game.game_over:
            ...
            # Switch player BEFORE updating visualization
            self.game.switch()
            
            # Update policy visualization if it's visible to show NEXT player's turn
            if self.visualization_visible and self.auto_update_var.get():
                self.current_vis_player = self.game.current_player  # Set to new current player
                self._update_visualization()
