    ...
    def _update_visualization(self):
        ...
        try:
            ...
            if hasattr(self, 'nn'):
                ...
                # Always display the value from current player's perspective (as returned by NN)
                # And indicate whose perspective it is in the value text
                if current_state.current_player == RED_PIECE:
                    value_perspective = "Red"
                    red_win_prob = (value + 1) / 2.0
                else:
                    value_perspective = "Green"
                ...
                # FIXED: Move Value text to upper right corner inside the frame
                # Show the value with an explicit perspective
                value_text = f"{value_perspective} Value: {value:.3f}"
                    
                ax.text(
                    7.7, 1.15 * max_height,  # Updated position as requested
                    value_text,
                    ha='right', va='top',
