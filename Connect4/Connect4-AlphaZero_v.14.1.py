...
class NNManager:
    ...
    def finish_game(self, winner):
        ...
        # Improved column balance pruning using KL divergence
        if len(self.data['states']) > 1000:
            self._balance_column_distribution()
        
        # Manage dataset size
        if len(self.data['states']) > MAX_TRAINING_EXAMPLES:
            self._prioritized_sampling()

    def _balance_column_distribution(self):
        """Balance column distribution using KL divergence"""
        ...
        uniform_prob = 1.0 / COLUMN_COUNT
        actual_probs = move_counts / len(policies)
        
        kl_divergences = []
        for i in range(COLUMN_COUNT):
            if i in unique_moves:
                idx = np.where(unique_moves == i)[0][0]
                prob = actual_probs[idx]
            else:
                prob = 1e-8  # Avoid log(0)
            
            kl_div = prob * np.log(prob / uniform_prob) if prob > 0 else 0
            kl_divergences.append(kl_div)
        
        # Apply continuous penalty based on KL divergence
        total_kl = sum(kl_divergences)
        if total_kl > 0.5:  # Threshold for rebalancing
            # Calculate keep probabilities
            keep_probs = {}
            for i, move in enumerate(unique_moves):
                # Higher KL divergence = lower keep probability
                normalized_kl = kl_divergences[move] / total_kl
                keep_prob = max(0.3, 1.0 - 2 * normalized_kl)  # Between 0.3 and 1.0
                keep_probs[move] = keep_prob
            
            # Apply filtering
            indices_to_keep = []
            for i, max_idx in enumerate(max_indices):
                if max_idx in keep_probs and random.random() < keep_probs[max_idx]:
    ...
    def train(self, batch_size=None, epochs=None, start_time=None, logger=None, num_games=1):
        ...
        # Enhanced dynamic weighting using gradient magnitudes
            ...
            for s, p_target, v_target in dl:
                ...
                if self.use_mixed_precision:
                    with torch.cuda.amp.autocast():
                        ...
                        # Create masks for invalid moves
                        batch_games = []
...
class TreeNode:
    ...
    def __init__(self, state: 'Connect4Game', parent=None, move=None, prior=0.0, original_prior=None):
        ...
        self.nn_version = None  # Initialize nn_version attribute
...
class MCTS:
    def __init__(self, iterations, c_puct, nn: NNManager, explore=True, dirichlet_noise=0.4, safe_bypass_prob=0.15):  # Increased from 0.3 to 0.4
        self.I = iterations
        self.c = c_puct
        self.nn = nn
    ...
    def search(self, root_state: 'Connect4Game', previous_root=None, move_number=0):
        ...
        if safe_moves and not bypass_safe_logic:
            if len(safe_moves) > 1:
                ...
                base_policy, _ = self.nn.policy_value(root_state)  # Fixed the typo here
        ...
        # Tree reuse with invalidation check
        ...
        # Get network policy and initialize
    ...
    def _calculate_safety_probability(self):
        """Calculate annealing safety probability based on training progress"""
        # Get total games from neural network if available
        total_games = 0
    ...
    def _normalize_reused_root(self):
        """Enhanced adaptive tree reuse normalization"""
...
class MCTSComputerPlayer(Player):
    def __init__(self, iters, c, nn: NNManager, explore=True, model_path=None, dirichlet_noise=0.4, safe_bypass_prob=0.15):
...
class Connect4GUI(tk.Tk):
    ...
    def _choose_players(self):
        ...
        # Create independent neural network managers for each player
        red_player = dlg.result['red']
        green_player = dlg.result['green']
        
        # Handle Red player neural network
        if isinstance(red_player, MCTSComputerPlayer):
            if hasattr(red_player, 'model_path') and red_player.model_path and red_player.model_path != self.nn_model_path:
                # Create independent neural network for Red player
                hyperparams = self.nn_params.copy()
                try:
                    model_data = torch.load(red_player.model_path, map_location=torch.device('cpu'), weights_only=True)
                    use_resnet = model_data.get('hyperparams', {}).get('use_resnet', False)
                    hyperparams['use_resnet'] = use_resnet
                except:
                    pass
                red_player.nn = NNManager(hyperparams, red_player.model_path, use_resnet=hyperparams.get('use_resnet', False))
                red_player.mcts = MCTS(red_player.mcts.I, red_player.mcts.c, red_player.nn, red_player.mcts.explore,
                                     dirichlet_noise=self.mcts_params.get('dirichlet_noise', 0.4))
            else:
                # Use shared neural network
                red_player.nn = self.nn
                red_player.mcts.nn = self.nn
        
        # Handle Green player neural network
        if isinstance(green_player, MCTSComputerPlayer):
            if hasattr(green_player, 'model_path') and green_player.model_path and green_player.model_path != self.nn_model_path:
                # Create independent neural network for Green player
                hyperparams = self.nn_params.copy()
                try:
                    model_data = torch.load(green_player.model_path, map_location=torch.device('cpu'), weights_only=True)
                    use_resnet = model_data.get('hyperparams', {}).get('use_resnet', False)
                    hyperparams['use_resnet'] = use_resnet
                except:
                    pass
                green_player.nn = NNManager(hyperparams, green_player.model_path, use_resnet=hyperparams.get('use_resnet', False))
                green_player.mcts = MCTS(green_player.mcts.I, green_player.mcts.c, green_player.nn, green_player.mcts.explore,
                                       dirichlet_noise=self.mcts_params.get('dirichlet_noise', 0.4))
            else:
                # Use shared neural network
                green_player.nn = self.nn
                green_player.mcts.nn = self.nn
        ...
        # Ensure window layout is preserved
        self.update_idletasks()
