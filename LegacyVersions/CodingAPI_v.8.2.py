...
def extract_json_output(text):
    ...
    Handles malformed JSON responses more gracefully.
    ...
    try:
        ...
        # Try to clean up the JSON string (some models add extra characters)
        ...
        try:
            ...
            # Try to extract corrected code directly from the text
            code_block_match = re.search(r'```.*?\n(.*?)```', text, re.DOTALL)
            if code_block_match:
                extracted_code = code_block_match.group(1).strip()
                return extracted_code, default_corrections
                
            # If still no success, try more aggressively to find code sections
            # Look for "corrected_code": "..." pattern
            code_match = re.search(r'"corrected_code"\s*:\s*"(.*?)"', text, re.DOTALL)
            if code_match:
            ...
            # If we reach here, return the raw text as code
