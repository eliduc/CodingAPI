...
def generate_with_claude(client, model, description, language, file_content=None, temp_allowed=True):
    ...
    try:
        ...
        # Add temperature parameter only if the model supports it AND thinking is not enabled
        # When thinking is enabled, we must omit temperature or set it to 1
        if temp_allowed and not is_claude_3_7_sonnet:
            kwargs["temperature"] = 0
...
def generate_with_gemini(client, model, description, language, file_content=None, temp_allowed=True):
    ...
    # Check if this is Gemini 2.5 Flash (for thinking_budget parameter)
    is_gemini_2_5_flash = model == "gemini-2.0-flash"
    ...
    try:
        ...
        # Add thinking_budget for Gemini 2.5 Flash
        if is_gemini_2_5_flash:
            config_params["thinking_config"] = types.ThinkingConfig(
                thinking_budget=24576  # Maximum thinking capacity
            )
...
def generate_with_deepseek(client, model, description, language, file_content=None, temp_allowed=False):
    ...
    try:
        ...
        # Note: DeepSeek R1 does not support temperature parameter according to documentation
        # So we don't add it even if temp_allowed is True
