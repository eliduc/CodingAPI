...
#!/usr/bin/env python3#!/usr/bin/env python3
"""
CodingAPI - Enhanced version 8.5
...
- Simplified UI by moving API Keys and Models configuration to Settings (new in v8.5)
...
def get_output_directory(project_name):
    ...
    """
    # Load settings
    config_file = os.path.expanduser("~/.codingapi/settings.json")
    settings = {}
...
def open_settings_dialog(root, config, update_callback, all_available_models):
    ...
    print(f"Loaded output directory from settings: '{current_output_dir}'")
    ...
    # API Keys & Models tab (combining the two previous dialogs)
    ...
    # Variables to store model selections
    coding_vars = {}
    auditing_vars = {}
    ...
    def create_provider_tab(provider_name):
        ...
        lbl_models_title = ttk.Label(frame, text="Selected Models:", style='Bold.TLabel')
        ...
        # Coding models frame
        coding_frame = ttk.LabelFrame(frame, text="Coding Models")
        coding_frame.pack(fill=tk.X, pady=5)
        
        # Create scrollable frame for models
        coding_canvas = tk.Canvas(coding_frame, height=100)
        coding_scrollbar = ttk.Scrollbar(coding_frame, orient="vertical", command=coding_canvas.yview)
        coding_scrollable_frame = ttk.Frame(coding_canvas)
        
        coding_scrollable_frame.bind(
            "<Configure>",
            lambda e: coding_canvas.configure(scrollregion=coding_canvas.bbox("all"))
        )
        
        coding_canvas.create_window((0, 0), window=coding_scrollable_frame, anchor="nw")
        coding_canvas.configure(yscrollcommand=coding_scrollbar.set)
        
        coding_canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        coding_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        # Add checkboxes for coding models
        for model in available_models:
            var = tk.BooleanVar()
            var.set(model in selected_models.get("coding", []))
            coding_vars[model] = var
            
            cb = ttk.Checkbutton(coding_scrollable_frame, text=model, variable=var)
            cb.pack(anchor="w", pady=2)
        
        # Auditing models frame
        auditing_frame = ttk.LabelFrame(frame, text="Auditing Models")
        auditing_frame.pack(fill=tk.X, pady=5)
        
        # Create scrollable frame for models
        auditing_canvas = tk.Canvas(auditing_frame, height=100)
        auditing_scrollbar = ttk.Scrollbar(auditing_frame, orient="vertical", command=auditing_canvas.yview)
        auditing_scrollable_frame = ttk.Frame(auditing_canvas)
        
        auditing_scrollable_frame.bind(
            "<Configure>",
            lambda e: auditing_canvas.configure(scrollregion=auditing_canvas.bbox("all"))
        )
        
        auditing_canvas.create_window((0, 0), window=auditing_scrollable_frame, anchor="nw")
        auditing_canvas.configure(yscrollcommand=auditing_scrollbar.set)
        
        auditing_canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        auditing_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        # Add checkboxes for auditing models
        for model in available_models:
            var = tk.BooleanVar()
            var.set(model in selected_models.get("auditing", []))
            auditing_vars[model] = var
            
            cb = ttk.Checkbutton(auditing_scrollable_frame, text=model, variable=var)
            cb.pack(anchor="w", pady=2)
    ...
    # ======================================================================
    # Create an explicit button frame at the bottom of the dialog
    # ======================================================================
    button_frame = ttk.Frame(settings_dialog, padding=(10, 5, 10, 10))
    button_frame.grid(row=1, column=0, sticky="ew")
    button_frame.columnconfigure(0, weight=1)  # Push buttons to the right
    ...
    # Print confirmation that buttons are created
    print("Settings dialog buttons created: Save and Cancel")
...
def main():
    ...
    def open_api_config(all_available_models=None):
        ...
        title_label.pack(anchor="w", pady=(0, 10))
        Tooltip(title_label, "Configure API keys and view available models for each provider")
        ...
        def create_provider_tab(provider_name):
            ...
            Tooltip(lbl_models, f"Models available with your current {provider_name} API key")
        ...
        Tooltip(btn_refresh, "Check for available models with current API keys")
    ...
    def run_iteration_loop(output_dir, base_filename, description, init_code, language, coding_llm, audit_llm, max_iterations):
        """
    ...
    def process_single_model(model_name, description, language, file_content, output_dir, result_queue):
        """
    ...
    def run_multiple_creation(output_dir, description, language, selected_models, file_content=None):
        """
        ...
        with ThreadPoolExecutor(max_workers=min(10, len(selected_models))) as executor:
            ...
            for model_name in selected_models:
                ...
                # Submit the task - Pass output_dir instead of filename
    ...
    def background_process_with_params(filename, description, language, coding_llm, audit_llm):
        ...
        try:
            ...
            if mode_var.get() == "creation":
                ...
                # Pass output_dir instead of filename
                run_multiple_creation(output_dir, description, language, selected_models, file_content)
                ...
                # Pass output_dir instead of filename
                run_iteration_loop(
                    output_dir, filename, description, init_code,
    ...
    def background_process_with_params(filename, description, language, coding_llm, audit_llm):
        ...
        try:
            ...
            if mode_var.get() == "creation":
                ...
                # Pass output_dir instead of filename
                run_iteration_loop(
                    output_dir, description, init_code,
                    language, coding_llm, audit_llm, max_iterations
                )
