...
"""
CodingAPI - Enhanced version 8.1.1
...
class SecureConfig:
    ...
    def _discover_claude_models(self, api_key: str) -> List[str]:
        ...
        try:
            ...
            # Claude doesn't have a models endpoint, so we'll check against known patterns
            # and test access to different models
            potential_models = [
            ...
            available_models = []
            for model_id, display_name in potential_models:
                try:
    ...
    def get_selected_models(self) -> Dict[str, List[str]]:
        ...
        """
        config_file = os.path.expanduser("~/.codingapi/model_selection.json")
        if os.path.exists(config_file):
            try:
                with open(config_file, 'r') as f:
                    return json.load(f)
...
def extract_json_output(text):
    ...
    Robust handling for multiple response formats and invalid JSON.
    ...
    # Check if this is just a Python code block without JSON wrapper
    if text.strip().startswith("```python") and "```" in text:
        try:
            # This is a direct code block, not JSON
            match = re.search(r'```python\s*(.*?)\s*```', text, re.DOTALL)
            if match:
                code = match.group(1).strip()
                return code, default_corrections
        except Exception as e:
            logger.warning(f"Failed to extract Python code block: {e}")
    ...
    try:
        ...
        try:
            with open(f"{debug_dir}/cleaned_json_{timestamp}.txt", "w", encoding="utf-8") as f:
                f.write(json_str)
        except Exception:
            pass
        ...
        # Step 1: Replace all known invalid escape sequences
        json_str = json_str.replace('\\FONT', 'FONT')  # The \FONT_NAME issue
        json_str = json_str.replace('\\F', 'F')        # Another case
        
        # Step 2: Systematically fix all invalid escape sequences
        # First, extract all string literals to avoid modifying escaped quotes
        string_literals = {}
        ...
        def replace_string_literals(match):
            ...
            string_literals[placeholder] = content
            placeholder_counter += 1
            return placeholder
            
        # Extract all string literals using regex
        pattern = r'"(?:[^"\\]|\\.)*"'
        json_with_placeholders = re.sub(pattern, replace_string_literals, json_str)
        
        # Fix backslashes in all extracted string literals
        for placeholder, string_literal in string_literals.items():
            # Step 1: Unescape valid escape sequences to preserve them
            temp = string_literal[1:-1]  # Remove outer quotes
            
            # Step 2: Replace invalid escapes with their character
            # Legal JSON escapes are: \", \\, \/, \b, \f, \n, \r, \t, \uXXXX
            # Replace any \X where X is not one of these
            temp = re.sub(r'\\([^"\\/bfnrtu])', r'\1', temp)
            
            # More aggressive: Handle any uppercase letter escape
            for char in 'ABCDEFGHIJKLMNOPQRSTUVWXYZ':
                temp = temp.replace(f'\\{char}', char)
                
            # Step 3: Re-escape valid escape sequences
            string_literals[placeholder] = f'"{temp}"'
        
        # Reconstruct the JSON string with fixed literals
        fixed_json_str = json_with_placeholders
        for placeholder, fixed_literal in string_literals.items():
            fixed_json_str = fixed_json_str.replace(placeholder, fixed_literal)
        
        # Save the fixed JSON for debugging 
        try:
            with open(f"{debug_dir}/fixed_json_{timestamp}.txt", "w", encoding="utf-8") as f:
        ...
        try:
            if not fixed_json_str.strip():
                logger.error("Error: Empty JSON string")
            ...
            # If JSON parsing fails, use a cascade of fallback methods
            ...
            if code_block_match:
                ...
                return extracted_code, default_corrections
            
            # Fallback 2: Extract any code block
            code_block_match = re.search(r'```.*?\n(.*?)```', text, re.DOTALL)
            if code_block_match:
                extracted_code = code_block_match.group(1).strip()
                return extracted_code, default_corrections
                
            # Fallback 3: Extract corrected_code field directly with regex
            code_match = re.search(r'"corrected_code"\s*:\s*"(.*?)"(?=\s*,\s*"corrections")', text, re.DOTALL)
            if code_match:
                extracted_code = code_match.group(1)
                extracted_code = extracted_code.replace('\\"', '"').replace('\\n', '\n')
                return extracted_code, default_corrections
            
            # Fallback 4: Look for Python code pattern
            python_imports = re.search(r'import\s+\w+', text)
            python_defs = re.search(r'def\s+\w+\s*\(', text)
            if python_imports or python_defs:
                # Try to extract a coherent Python module
                return text, default_corrections
                
            # Last resort: return the text as-is
            return text, default_corrections
        
        # Extract code and corrections from parsed JSON
        if isinstance(result, dict):
            # Get code from the 'corrected_code' field
            code = result.get('corrected_code', '')
            # If code is a string with escaped newlines, replace them
            if isinstance(code, str):
                code = code.replace('\\n', '\n').replace('\\"', '"')
            
            # Get the corrections array
        ...
        # Try to find a code block as a last resort
...
def main():
    ...
    btn_config = ttk.Button(frm_buttons_top, text="API Keys and Models", width=20)
    btn_check_models = ttk.Button(frm_buttons_top, text="Check Models", width=15)
    ...
    btn_check_models.pack(side=tk.RIGHT, padx=5)  # Check Models next to Exit
    btn_config.pack(side=tk.RIGHT, padx=5)  # API Keys and Models next to Check Models
    ...
    def open_api_config(all_available_models=None):
        """Open a dialog to configure API keys for different providers."""
        config_window = tk.Toplevel(root)
        config_window.title("API Keys and Models")  # Changed title
        ...
        # Title
        title_label = ttk.Label(main_frame, text="API Keys and Models", font=("Arial", 14, "bold"))  # Changed title
        title_label.pack(anchor="w", pady=(0, 10))
        ...
        def create_provider_tab(provider_name):
            ...
            # Use pre-discovered models if available, otherwise fall back to API call
            if all_available_models:
                available = all_available_models.get(provider_name, [])
            else:
                # Fallback to API call (for refresh button)
        ...
        # Refresh button - re-discovers models
        def refresh_models():
            ...
            # Update all available models by doing full discovery
            print("Re-discovering available models...")
            new_all_available_models = config.get_all_available_models_from_api()
            
            selected_models = config.get_selected_models()
            
            # Update the main window's model variables
            # This is a bit hacky but necessary to update the parent scope
            nonlocal all_available_models
            all_available_models = new_all_available_models
            
            # Filter selected models to only include available ones
            new_available_coding_models = [model for model in selected_models.get("coding", []) 
                                         if any(model in new_all_available_models.get(provider, []) 
                                               for provider in new_all_available_models)]
            new_available_auditing_models = [model for model in selected_models.get("auditing", []) 
                                           if any(model in new_all_available_models.get(provider, []) 
                                                 for provider in new_all_available_models)]
            
            # If no selected models are available, use all available
            if not new_available_coding_models:
                new_available_coding_models = [model for provider_models in new_all_available_models.values() 
                                             for model in provider_models]
            if not new_available_auditing_models:
                new_available_auditing_models = [model for provider_models in new_all_available_models.values() 
                                               for model in provider_models]
            
            if not new_available_coding_models:
                new_available_coding_models = ["No models available - configure API keys first"]
                
            if not new_available_auditing_models:
                new_available_auditing_models = ["No models available - configure API keys first"]
            
            # Update comboboxes in main window
            combo_coding_llm['values'] = new_available_coding_models
            combo_audit_llm['values'] = new_available_auditing_models
            
            if new_available_coding_models and new_available_coding_models[0] != "No models available - configure API keys first":
                combo_coding_llm.current(0)
                
            if new_available_auditing_models and new_available_auditing_models[0] != "No models available - configure API keys first":
                combo_audit_llm.current(0)
                
            # Update listbox
            lb_coding_llm.delete(0, tk.END)
            for model in new_available_coding_models:
                lb_coding_llm.insert(tk.END, model)
                
            # Refresh the dialog
            config_window.destroy()
            open_api_config(new_all_available_models)  # Pass the updated models
    ...
    def run_iteration_loop(filename, description, init_code, language, coding_llm, audit_llm, max_iterations):
        ...
        for i in range(1, max_iterations + 1):
            ...
            try:
                ...
                # Debug print to log the actual parsed counts
                ...
                try:
                    if filename:
                        ...
                        if is_final:
                            audit_file = os.path.join(filename, f"{filename}_audit-Final.txt")
                        else:
            ...
            try:
                ...
                # FIX: Only use the extracted code string, not the JSON response
                if isinstance(corrected_code, str):
                    # Check if the corrected code might be a JSON string
                    if corrected_code.strip().startswith('{') and corrected_code.strip().endswith('}'):
                        try:
                            # Try to parse as JSON
                            json_data = json.loads(corrected_code)
                            # If successful, extract the "corrected_code" field
                            if isinstance(json_data, dict) and 'corrected_code' in json_data:
                                corrected_code = json_data['corrected_code']
                        except:
                ...
                try:
                    if filename:
                        ...
                        if is_final:
                            iteration_file = os.path.join(filename, f"{filename}-Final{ext}")
                        else:
    ...
    def run_multiple_creation(filename, description, language, selected_models, file_content=None):
        ...
        if completed_models_with_time:
            ...
            try:
                if filename:
                    ext = extension_for_language(language)
                    final_file = os.path.join(filename, f"{filename}-Final{ext}")
    ...
    def open_api_config_with_models():
        open_api_config(all_available_models)

    btn_config.config(command=open_api_config_with_models)
    
    # Set command for Check Models button
    def open_check_models():
        open_model_selection_dialog(root, config, update_model_lists, all_available_models)

    btn_check_models.config(command=open_check_models)
    
    # Connect Clear button to clear function
    btn_clear_desc.config(command=lambda: text_description.delete("1.0", tk.END))
